 ...
 *
 * Each builder returns the full operational report. `toPublicDataStatus` is the
 * only sanctioned way to expose any of it to an unauthenticated product client:
 * it drops provider/source names, per-symbol provenance and coverage gaps, and
 * keeps just the symbol registry the product itself renders.
 */
...
/**
 * The product only needs to know which symbols exist and whether coverage is
 * healthy overall. Provider names, per-symbol provenance, coverage gaps and
 * unregistered extra symbols stay in the admin report.
 */
function toPublicDataStatus(report) {
  return {
    ...
    universe: { scan_enabled_count: report.universe?.scan_enabled_count ?? null },
  };
}

async function buildOptionsStatus() {
  const watchlist = loadWatchlist();
