...
The first manual cloud execution reached PostgreSQL and loaded the 67-symbol watchlist. Investigation found a state-handling defect: a successful TT session exchange can include a successor `remember-token`, but the prior collector retained only the session token. Its next one-shot invocation then used an obsolete credential and failed. `auth.py` now accepts a session only after atomically persisting the provider-supplied successor. It never creates a token, retries a failed login, or overwrites state on 401/403. Railway Cron must mount a persistent `/data` volume, set `TT_REMEMBER_TOKEN_STATE_PATH=/data/tastytrade-remember-token`, and enable `TT_REMEMBER_TOKEN_STATE_REQUIRED=true`; without its volume mount, the job fails before it contacts TT. The service variable remains the initial seed, while the mounted file is the durable successor state. Cloud collection remains incomplete until a new manual `auth.py --login` seed is supplied, then one manual run updates `iv_history`.
