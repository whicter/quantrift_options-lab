...
- Cloud refresh cron artifacts are `Dockerfile.metrics` and `railway.metrics.json`. Railway service `quantrift-metrics-cron` uses `/collector/railway.metrics.json`; inject DB variables in the service and never copy `.env`. The weekday `*/5` UTC job runs `run_railway_refresh_cycle.py` once: it schedules bounded watchlist refreshes, consumes `provider_fetch_jobs`, then materializes scanner rows. TT metrics remain disabled in this cloud runtime. The build context is repository root, so the Dockerfile must copy `collector/requirements.txt` and `collector/`, not paths relative to the config file.
