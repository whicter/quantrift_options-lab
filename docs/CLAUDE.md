...
- `/` owns the Quantrift product entry with live Market Regime and direct core workflows. Keep `/learn` as a separate education route. `/market` is the decision-language hub (R1.1/R2.2): options-native breadth panel + Symbol State Matrix, with R1.3 rotation and R1.2 briefing to join it. The breadth panel lives on `/market`, not home.
