...
- [x] Weekly 真实数据：`/api/weekly/:symbol` 返回 rolling 5-session OHLC、每日实际 GEX history、Max Pain、ΔOI 与条件剧本；`weeklyMock.js` 已删除
...
- [x] Sec4 仓位变化：真实 ΔOI 日汇总；明确不将 OI 变化伪装成美元资金流或机构方向
- [x] Sec5 下周分叉：多头/空头剧本卡片（触发条件/价格目标/观察重点）
- [x] index.css：新增 ~170行 Phase 1 样式 + ~200行 Phase 2 样式（.wk-* 类）
- [x] /weekly 全量数据化：不再按 symbol fallback mock；每个 module 对真实字段独立 fail closed
...
- [x] Production API verified after deploy：2026-07-15 `GET /api/prices/AAPL?limit=3` 返回 HTTP 200、`freshness=fresh`
...
- [x] Add 10 more strategies (exotic, FX, index-specific)（策略库增至 88 个模板：Call/Put Ladder、比例日历、Calendar Condor、Double Diagonal Condor、FX Risk Reversal / Seagull、Index Iron Condor / Broken-Wing Butterfly；catalog 测试校验数量、ID 唯一和新增模板存在）
- [x] 策略 notes 进一步标准化（所有 88 个策略的 `iv` / `dte` / `tp` / `sl` 均展示至少一个数字阈值；模板本身已有数字时保留原规则，缺失项补入统一的 IV Rank 30-60、30-60 DTE/45 DTE、50% 止盈和 50% 最大风险止损基准；单元测试逐策略校验）
...
- [x] 服务层自动切换：derived IV Rank ready 后 API/scanner 使用 derived；batch collector、on-demand API 与 refresh worker 均停止为该 symbol 调用 Tastytrade（2026-07-15）
...
- [ ] Tastytrade collector 迁移：从 Mac Studio 搬到 Railway Cron Job（纯 REST API，无需本地网关，可直接云端跑）
  ...
  - [x] Secret contract：`DATABASE_URL`、`TT_LOGIN`、bootstrap `TT_REMEMBER_TOKEN`（仅在 Railway 自己的 `provider_auth_state` 尚无记录时使用）、`TT_BASE_URL`、`TT_USER_AGENT`；镜像排除 `.env` 与本地 venv。`auth.py --login` 只会 seed 到该进程绑定的数据库；只有相同 `DATABASE_URL` 才与 Railway 共享。Railway 不需要接收 successor token。缺 `TT_LOGIN` 时 collector 必须在本地 fail closed，不能向 TT 发送请求。
  - [x] Token-state durability：新表 `provider_auth_state(provider, remember_token, updated_at)` 是本机与 Railway Cron 的唯一续期状态。每次 exchange 先取得 PostgreSQL transaction advisory lock，201 后在同一事务写入 successor/当前 token；401/403/网络失败 rollback，不改状态。若数据库 token 明确 401/403 且 Railway 配置的 seed 不同，只以该 seed 额外恢复一次，成功后才覆盖旧状态；不进行密码 fallback。无需 Railway `/data` Volume。
  ...
  - [ ] 首个成功 cloud run：2026-07-16 手动 run 已确认容器连通 PostgreSQL、加载 67 个 symbols。首次失败发现 Railway 缺少 `TT_LOGIN`，已补入并部署；随后单次 execution 仍明确返回 `401 invalid_credentials`，数据库旧 token 与 configured seed 都被 TT 拒绝。该结论不归因于 Railway 网络、Docker 或 PostgreSQL，且 collector 已停止，不会再重试。111 个 collector 测试已通过。完成条件：账户持有人提供有效且匹配的现有 `TT_LOGIN`/`TT_REMEMBER_TOKEN` 组合后，单次 Railway execution log 显示成功 authentication/写入，并验证新增或更新 `iv_history` rows 与 Railway 数据库的 `provider_auth_state.updated_at`。
...
- [x] Scanner strategy recommendation expansion（由 Phase 3H-1/3H-2 与 P1.1 完成）：
...
- [ ] 满 252 个独立交易日后自动使用 derived IV Rank 并停止 Tastytrade iv_rank；当前 1–2 observations/symbol、0/67 ready，继续使用 TT 冷启动值，不提前伪造 readiness
  - [x] 自动切换代码已完成：`collect.py` 在认证前过滤 ready symbols；refresh worker 对 ready symbol 返回 `already_ready`；Analyze 不再 enqueue TT metrics job
  - [x] Tests：server 40/40、collector 81/81；synthetic 252 observations 已验证 rank readiness，worker/collector/API 已验证 provider cutoff
...
**P1 — 数据已有，可立刻做**
1. [x] Screener 策略扩展：13 种真实合约结构与风险门控已完成
2. [x] S/R 端点：server 新增 `GET /api/sr/:symbol` + Tab2/Tab4 K线图叠加支撑压力水平线
3. [x] Scan 页顶部 Market Regime Header（SPY/QQQ GEX regime + IV Rank；VIX 无同口径数据时不伪造）

**P2 — 需要小改后端**
4. [x] 非 watchlist 标的按需 enqueue + 前端等待/blocker UI（`/api/analyze/:symbol` 对未知 symbol 触发完整数据 bundle）
...
- [x] API 对未知标的注册 persistent universe，并 enqueue metrics/price/options/GEX/scanner bundle；返回 product-level ready/queued/blocked
- [x] 前端显示数据采集状态、已有 partial products 与 non-retryable blocker；PostgreSQL snapshot 是缓存层
- 无需为此修改数据库 schema
...
- [x] `/api/chain/stats/:symbol` 独立选择最新含 OI 的 snapshot；跨未到期 expiry 聚合 `call_oi` / `put_oi` / `total_oi` by strike，返回独立 source/freshness
...
- [x] Analyze Tab4：主力持仓密度使用真实 OI by strike 堆叠显示 Put/Call；缺 OI 时 unavailable，不回退 GEX
...
- [ ] User authentication（Clerk）
  - [x] Express `clerkMiddleware`、JSON 401/503 guard、authorized parties、`/api/account/me`
  - [x] React conditional `ClerkProvider`、SignIn/UserButton、`/account` route；无 key 时不挂载 SDK
  - [x] Tests/build：server 43/43、frontend 19/19、Vite build passed
  - [ ] Railway/Vercel 注入 Clerk publishable/secret keys 并完成真实 sign-in 验收
- [ ] 订阅分层: 免费（教育工具）/ 付费（scanner + alerts + live data）
  - [x] Free/Pro plan catalog 与 bounded entitlements 已写入 account API
  - [x] scanner/alerts/live/portfolio routes 已接入 entitlement middleware；`AUTH_ENFORCEMENT_ENABLED=false` rollout gate
  - [x] Frontend API bridge 为所有数据请求附加 Clerk bearer token，避免登录用户仍被 401
  - [ ] 配置 Clerk/Stripe、完成 webhook lifecycle 后将 production enforcement 改为 true
- [x] Portfolio dashboard：Clerk-owned CRUD、multi-leg positions、真实 contract snapshot mark、实际 P/L 与 aggregate Delta/Gamma/Theta/Vega（2026-07-15）
  - [x] Missing quote fail closed：position/summary 显示 `待报价`，不以 entry price 伪装 current mark
  - [x] Ownership：所有 list/create/close SQL 绑定 authenticated local user id
  - [x] Tests/build：server 46/46、frontend 21/21、Vite build passed
- [x] PostgreSQL: users / subscriptions / positions 表
  - [x] `users` / `subscriptions` additive schema 已实现；新 Clerk user 自动获得 free subscription
  - [x] Railway migration：2026-07-15 执行成功；只读确认 `users`、`subscriptions`、`positions`、`position_legs`、`stripe_webhook_events` 全部存在
  - [x] `positions` / `position_legs` schema 与 ownership indexes 已实现
- [ ] Payment integration（Stripe）
  - [x] Checkout subscription、Customer Portal、signed raw-body webhook、Free/Pro lifecycle mapping
  - [x] `stripe_webhook_events` event-id idempotency；plan 只由 signed subscription webhook 更新
  - [x] Tests/build：server 56/56、frontend 21/21、Vite build passed
  - [ ] 提供 `STRIPE_SECRET_KEY`、`STRIPE_WEBHOOK_SECRET`、`STRIPE_PRO_PRICE_ID` 并完成 test-mode checkout/webhook/portal 验收
- [x] Custom domain 配置：`quantrift.io` 308 → `www.quantrift.io`，www HTTP 200
