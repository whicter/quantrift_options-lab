...
- [x] Railway migration（2026-07-17）：对生产 Railway PostgreSQL（`tokaido.proxy.rlwy.net`，pg 18.4）跑 `node src/migrate.js`，两张新表 + 全部字段 + 索引（`scanner_candidate_batches_key_status_completed`、`scanner_candidate_snapshots_batch_rank`、`_batch_symbol`、`UNIQUE (batch_id, candidate_key)`）确认创建；`scanner_results_snapshots` 未受影响。
- [x] Runtime acceptance（2026-07-17，生产数据实跑）：`node src/jobs/materializeScannerCandidates.js watchlist_v1` → batch id=1，`universe_count=81`、`candidate_count=4768`、`status=completed`、`source_snapshot_cutoff=2026-07-17T19:25:16Z`。读路径查询（latest completed batch + rank 排序）返回真实候选：rank-1 = MSFT Diagonal Spread（Sell 08-21 450C 35DTE / Buy 09-18 390C 63DTE，真实 bid/ask/delta），family 分布 time_spread 2822 / iron 642 / single_leg 556 / combo 386 / credit_vertical 244 / straddle_strangle 118。
- [x] Collector 调度接入（E15，2026-07-17）：新增 `collector/materialize_scanner_candidates.py`——薄 wrapper，subprocess 调 `node server/src/jobs/materializeScannerCandidates.js`（候选引擎单一事实源在 JS，Python 不重写评分逻辑）。node 缺失 / 脚本缺失 / 无 `DATABASE_URL` / 超时 / 非零退出一律 warning 跳过，不炸 cycle。接入 `run_collector_daemon.py`（`materialize_scan.run()` 之后，同 `SCAN_MATERIALIZE_SECONDS=300` 节奏）与 `run_railway_refresh_cycle.py`（`materialize_scan.run()` 之后）。Env flag `SCANNER_CANDIDATE_MATERIALIZE_ENABLED`（默认 true）、`SCANNER_CANDIDATE_MATERIALIZE_TIMEOUT`（默认 180s）。Railway cron 镜像是 `python:3.11-slim` 无 node，会安全跳过；**Mac Studio daemon 有 node、是实际产出批次的 runtime**（每 5 分钟一次）。Tests：`collector/tests/test_materialize_scanner_candidates.py` 7 个（disabled 跳过、node 缺失跳过、无 DATABASE_URL 跳过、正确 argv 调用、非零退出被吞、脚本路径、cycle 顺序）。验证：collector `unittest discover` 194/194（130 → 194）；真实端到端 `python materialize_scanner_candidates.py` → 经 node 产出 batch id=2、81 标的、4768 候选、completed。
- [ ] 后续（未做，需人工浏览器验收）：前端接入 `/api/v1/scanner/candidates`（与 `/api/scan` 切流一并，属 rollout 后续步骤；Analyze/Scanner 页渲染无法在本环境自动截图验证，按项目标准需人工浏览器验收）。
- [x] 批次 retention（E16，2026-07-17）：`runMaterialization` 完成后调 `pruneOldBatches`——每 `scan_key` 只保留最新 `SCANNER_CANDIDATE_BATCH_KEEP`（默认 5）个 completed 批次，其余（旧 completed + 残留 running/failed）删除，`ON DELETE CASCADE` 连带清 snapshots；从不删刚写入的批次；prune 失败被吞不影响已完成批次。防止表无界增长（每 5 分钟一批 × ~4768 行）。`keep<=0` 关闭。Tests：materializer +3（keep N 保留+豁免当前批、keep≤0 关闭、失败返回 null），server `node --test` 151/151（148 → 151）。真实验证：`SCANNER_CANDIDATE_BATCH_KEEP=1` 跑一次 → `prunedBatches=2`，库中仅剩最新批次、snapshot 无孤儿行。
