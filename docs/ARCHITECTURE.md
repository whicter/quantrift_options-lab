...
The first manual cloud execution reached PostgreSQL and loaded the 67-symbol watchlist. Railway initially omitted `TT_LOGIN`; this is now a required credential-contract field and the collector fails before sending an HTTP request when it is absent. After `TT_LOGIN` was added and deployed, both durable Railway state and the configured `TT_REMEMBER_TOKEN` seed received explicit `401 invalid_credentials`, so the collector stopped without modifying state. The additive migration was applied on 2026-07-16. `provider_auth_state` is shared only by collectors using the same `DATABASE_URL`: a local `auth.py --login` does not seed Railway if its database binding differs. Railway bootstraps an empty row from `TT_REMEMBER_TOKEN`; every collector locks its provider row for the exchange and commits any successor before accepting the session. On an explicit 401/403 from a database token, and only when the configured seed differs, it makes one recovery exchange with that seed and commits only a successful result. It never creates a token, retries a password login, or retries transient errors. This avoids a Railway Volume entirely. Cloud collection remains incomplete until its execution log and resulting `iv_history`/`provider_auth_state.updated_at` are verified.
