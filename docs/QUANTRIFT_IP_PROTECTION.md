...
- [ ] 历史提交 secrets 审计完成。
- [x] production source maps 检查完成：`frontend/vite.config.js` 明确 `build.sourcemap=false`；2026-07-16 production build artifact 无 `.map` 文件。debug/internal endpoint 拆分仍在 V3A 后续任务。
- [x] Scanner 核心候选算法确认只在后端执行：普通 `/api/scan` 不返回完整 raw option chain；前端不再包含候选枚举、评分或策略经济性引擎。
