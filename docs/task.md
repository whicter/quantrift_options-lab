...
- [x] **PM2 cron 曾整体失效**：守护进程 2026-09-23 07:41 重启后，14 个 quantrift cron app 里
  ...
  **教训**：`pm2 save` 之后仍可能在守护进程重启时丢掉 cron 注册，所以"进程还在列表里"
  ...
  而不是看 `pm2 list` 的状态列。
- [x] **重新注册后出现了双写者，已清除（2026-09-24）**。07:41 那次守护进程重启丢了两样东西：
  cron 注册（上面修了）和**运行中的进程句柄**（当时没发现）。三个常驻进程的旧副本
  （PID 988 `collect_unusual_whales` / 1000 `run_collector_daemon` / 1001 `run_quote_worker_daemon`，
  均 PPID=1、起于 03:16 开机）脱管后继续运行，`pm2 delete` 碰不到它们；16:20 重新注册之后
  每个常驻进程都有两份——CLAUDE.md 禁止的单写者破坏。暴露它的是告警指纹：
  新代码对「完整度+过期」这组 issue 一律算出 `07d0eea6`，库里却每 5 分钟一个新指纹，
  只能来自内存里还是合并前代码的进程。已 SIGTERM 三个孤儿，确认只剩 PM2 管理的单实例，
  无遗留 `running` job。
- [x] **完整度告警改为按比例触发**：`HEALTH_MAX_INCOMPLETE_PCT=2`。FBND（平均 70.8%）
  与 SRVR（73.1%）每次快照都低于 75%，按"任意一个就报"会每小时响一次、永不消失；
  2/330 = 0.6%。名单仍写进报告（`incomplete_symbols`），只是不再升级成告警。
...
```
08-31   724 次查询 / 1 次命中 / miss 原因全部 no_rows / 同时写入 45,721 行
09-02   729 次查询 / 1 次命中
```

写入一直正常。问题在访问模式:**141 个标的每天各被访问恰好 1 次**，
而我把作用域定成「一个交易日」——于是读永远发生在那个标的当天唯一一次写之前。
按天过期是叠在充分性规则上的第二层保险，而恰恰是这层保险吃掉了全部收益。

**成本归属现在是直接计时，不是回归**（中位任务 54.7s）：

| 阶段 | 秒 | 占比 | 实测关系 |
|---|---|---|---|
| 合约发现 | 22.1 | 40% | 每次往返 `3.35 + 0.014×梯子`，6 次/标的 |
| subscribe | 19.9 | 36% | `0.193 × 合约数`，R²=0.992 |
| batch wait | 7.5 | 14% | 其中约 4.9s 是 OI 缺席的批次 |
| 其余 | ~5.0 | 10% | 连接 / 现价 / secdef / 落库 |

**修法**：读取改成「backstop 窗口内最新的一条梯子」。守正确性的是两条检查，都不是日历：

1. **`reqSecDefOptParams` 返回的实时行权价集合**——每次取链本来就会请求，现在传下去和
   缓存梯子比对。**这才是跨天安全的依据**：拆股不是在边缘加行权价（那样几何规则看得见），
   而是整条梯子替换，旧 conId 仍能解析成调整后的非标准合约——报出来的是**别的东西的
   合理数字**，比报不出来更糟。比对带容差，因为 IB 线格式和 `NUMERIC(18,4)` 不精确往返，
   严格相等会把每条梯子都判成过期、把缓存静默关掉。没有实时集合时跳过检查而不是假设。
2. **原有几何规则不变**：横跨窗口，或两侧都凑满 `max_per_side`。

`IB_OPTION_CONTRACT_CACHE_MAX_AGE_DAYS`（30）只是兜底，不是调优旋钮——这些期权本来
90 天内就到期。**没动任何既有阈值或间隔。**

实测：库里那条 SNDK 梯子现在 0.198s 返回 328 行，之前同一次读取返回空。

**同时更正两个我说得比证据更笃定的判断：**

- **OI 不是迟到，是缺席。** 来的时候是和报价一起来的（`oi_ready` 0.925s vs
  `quote_ready` 0.950s）。成本全在 358 批里那 137 批它压根没来、批次吃满窗口：
  6.083s vs 1.012s。移出完成判据值约 4.9s/标的——真实，但是 9%，不是我暗示的大头。
- **`IB_OPTION_CONTRACT_DELAY` 只解释 subscribe 的 26%。** 每合约 0.193s 对配置的
  0.05s sleep，其余是 `reqMktData` 本身，所以 delay 调到 0 最多省 19.9s 里的 5.2s。

- [ ] 下个交易日确认 `contract_cache_hits` 真的起来，并读出 discovery `source: cache` 的耗时
- [ ] 待定：把 OI 移出完成判据（逻辑改动，约 9%）
- [ ] 待定：`IB_OPTION_CONTRACT_DELAY` 0.05 → 0（参数改动，上限 5.2s，需试 IB 限速容忍度）
...
其余全在睡自己的限速器；限速行 40 秒墙钟推进 49.5 秒（**124%，已饱和排队**）。

**并发不提速**——`provider_rate_limits` 全局共享，N 个 worker 是把同一份预算切成
N 份而非乘以 N：1 个在跑 44s / 3 个 229s / 9 个 303s，**总吞吐完全相同**。
`REFRESH_WORKER_CONCURRENCY=3` 对速度是 no-op。

`POLYGON_OPTIONS_REQUEST_DELAY` 1.5s → 0.5s：

| 设置 | 均值 | 最快 | 吞吐 | 429 |
|---|---|---|---|---|
| 1.5s | 300s | 44.6s | 60/小时 | — |
| 0.3s | 56s | 8s | 172/小时 | 20 分 4 次 |
| **0.5s（采用）** | **43s** | **13s** | **137/小时** | 8 分 1 次 |

0.3s 吞吐更高但**延迟更差**——429 的 60s 全局罚时吃掉收益，0.5s 是延迟最优点。

**推翻旧结论**：先前「options 端点无速率上限」来自短时突发探测，而突发不是限速探测。
实际上限约在 120–200 请求/分。

**IB 报价** `IB_OPTION_QUOTE_BATCH_SIZE` 40 → 90：订阅一批等一个固定 6s 窗口，
180 合约 ÷ 40 = 5 批 × 6s = 30s，占了 41s 的大头；等待是**超时不是往返**，
所以批越大越好（IB 每账号约 100 条线）。预计 41s → 20s，**未验证**——
改动落在周五盘后，需下一个交易日实测。

仍未做：50 个请求换 144 个存储合约依然浪费，但收窄窗口只从 40 降到 33
（请求数被到期日数主导，不是窗口宽度），真正解法与覆盖率是同一个——按标的自适应。
...
中位 99.1%（门槛 90），最少计入 5,064 个标的，零质量失败、零算术失败。
`status: failed` 的唯一原因是**不完整**：

```
sessions: 180   范围 2024-08-16 ~ 2026-08-14
missing_weekdays: 341   (2024=97, 2025=237, 2026=7)
chain_breaks: 2025-11-26 的前一个存储会话是 2024-08-16
```

库里实际是「2024-08-16 一个孤点」+「2025-11-26 起的连续块」。
grouped daily 是滚动两年窗口，两年约 500 个交易日，我们只有 180。

回填已启动，预计约 7 小时。详见
`docs/validation/BREADTH_BACKFILL_VERIFY_2026-08-15.md`。

**在回填完成前不要把这张表当两年历史用**——NYMO 这类指标要连续序列，
180 个带洞的会话算出的 EMA 是错的。

### 过程中踩到的两个坑

1. **后台脚本不继承 PM2 的 env。** 第一次 `nohup` 启动只带了 PYTHONPATH，
   `POLYGON_REFERENCE_REQUEST_DELAY=3` 只写在 ecosystem 里，独立脚本拿不到，
   reference 回落到 16s：每会话 7 个请求＝112s，实测每会话约 190s；
   显式带 env 后降到约 72s。与 `backfill_iv_history.py` 是 bypass path
   属同一类问题——**新写的任何直连 provider 的脚本都要显式确认自己的 pacing**。
2. **16s 间隔的 grouped daily 在持续负载下仍会 429。** 无并发写入者时，
   6 分钟内仍有 2 次 60s 惩罚。之前用短时突发探测得到的「约 5 req/min」
   在持续负载下偏乐观。限速器行为正确（penalize 让所有 worker 一起退避、
   60s 自清），回填不会失败只会变慢，但**短时探测不等于限速探测**这条
   在 `probe_rate.py` 文件头写过的教训又应验了一次。

另记一个有界浪费：缺口列表按**工作日**生成而工作日含假期，
假期 walk-back 到的前一交易日通常已在库里，于是花整个周期幂等重写一行。
2026 的 7 个「缺失工作日」全是这种情况，所以 sessions 只从 179 涨到 180
而实际写了 7 次——**看进度时「写入次数」不等于「新增会话数」**。
2024–2025 的 334 个缺口是真实缺失，假期约 18 个，浪费约 5%，不值得改代码。
...
（completeness 100%、缺失希腊字母 0、缺失 OI 0），唯一不达标的是**上方**行权覆盖
1.8% < `MIN_WALL_COVERAGE_PCT=3.0`。`confidence_for` 里 `wide_enough` 同时是
`high` 和 `medium` 的前置条件，所以直接落 `low`，没有中间档。

**根因**：`OPTION_MAX_CONTRACTS_PER_BUCKET=250` 用**合约数**限制每个 DTE 桶，
但意图是 `OPTION_MAX_EXPIRATIONS_PER_BUCKET=2`（**到期日数**）。合约密度全universe
差约 20 倍，SPY 单个到期日在 ±15% 窗口内就约 500 个合约，250 在**第一个到期日内部**
就用尽。而 Polygon 对一个到期日是「先全部 call 按行权价升序，再全部 put 升序」，
所以截断必然落在某个 right 内部，砍掉的全是高行权价——实测 2026-11-30 拿到
137 个完整 call（660→890）但只有 19 个 put（660→682）。`_apply_strike_limit`
随后忠实地取「距现价最近 6 个」，而那组 put 里最近的也只到 682。

**这不是数据缺失，是采集边界被当成了市场结构**，所以 `low` 标签本身诚实，错的是采集窗口。

**修复**（`collector/providers/polygon_option_chain_provider.py`）：按到期日数分页
（看到第 N+1 个开始即证明前 N 个完整），合约数降级为翻页兜底（250→2000）；
兜底触发时丢弃末尾不完整的到期日（除非它是该桶唯一的）；`_trim_across_expiries`
增加 `spot`，到期日内按距现价距离取而非取列表头部。

**实测**（生产参数，下方/上方/到期日）：

| 标的 | 修复前 | 修复后 |
|---|---|---|
| SPY | 12.2% / 1.8% / 9 | 1.5% / 1.8% / 14（偏斜消除） |
| GLD | 13.32% / 0.63% / 4 | 2.9% / 3.4% / 12 |
| IWM | 10.19% / 0.95% / 4 | 3.3% / 4.9% / 12 → **达标** |

到期日 4→12 同时打击了最大的失败类别：310 个标的中 123 个原本是**因到期日不足 4**
不达标，而非行权覆盖。collector 全量 469 测试通过，新增 3 项回归测试。
耗时 41–62s/标的，与既有约 44s 基线相当。详见
`docs/validation/GEX_STRIKE_COVERAGE_SKEW_2026-08-15.md`。

### 仍未解决（同一种错误的另两种形态：用绝对量当百分比的代理）

1. **高价密集行权标的**（SPY 1.5%、GS 1.4%、COST 1.2%、SNDK 1.3%）：
   `OPTION_MAX_STRIKES_PER_SIDE=6` 是个数，SPY 在 $1 间距下 6 档只有 ±0.4%。
   达到 3% 需每边约 23 档＝每到期日 92 个合约（现 12），叠加 14 个到期日与
   `OPTION_MAX_CONTRACTS=180` 冲突。**这是 GEX 要行权宽度、策略引擎要到期日宽度
   之间的真实资源冲突**，而 `option_contract_snapshots` 已 653 MB
   （2026-07-30 曾因卷满宕机），需单独决策后再动。
2. **低价标的**（RR $1.80、AMC $2.50、REI $1.37）：±15% 的**百分比**窗口比**离散**
   行权间距还窄（$1.80 的 15% ＝ $0.27，间距 $0.50），窗口内只落进 1 档行权价。

两者都应改为按标的自适应，`adaptive_oi_window_pct` 已有现成范式可复用。

---

## ✅ 2026-08-15 — IB 报价改批量订阅：实测 17 倍，报价覆盖不再受限

**起因**：用户问「我自己交易时 IB 拿数据那么快，为什么采集要 165 秒一个标的」。查下来**慢的不是 IB，是我们的适配器**。

### 真因

`ib_option_chain_provider` 逐个合约订阅：`reqMktData` → 最多等 `IB_OPTION_STREAM_TIMEOUT` → `cancelMktData` → 下一个。
180 个合约就是 180 轮串行往返。**IB 每账户约 100 条行情线路，我们同一时刻只用 1 条。**
TWS 手动看链是瞬时的，因为它一次把整条链全订上。

同仓库的 `ib_news_provider` **早就是批量的**，注释写明「受行情线路上限约束，所以分批订阅，
批内并行而不是每标的付一次往返」——期权链这条路径从来没按这个模式写过。

### 实测 A/B（同 40 合约、同休市条件）

```
串行 (batch=1，等价旧行为)   249.5s
批量 (batch=40)              14.4s     → 17.3 倍
```

按此重算盘中单轮能力（休市保守估计，盘中有报价流时批次会提前退出，实际更快）：

```
串行 + 120 合约（改前）   165s/标的   390 分钟单轮  142 个
串行 + 180 合约           247s/标的   390 分钟单轮   95 个
批量 + 180 合约（现在）    65s/标的   390 分钟单轮  361 个   ← 超过全部 327 个 scan 标的
```

- [x] `fetch_contract_snapshots()` 批量订阅，`_build_snapshot()` 拆出复用；`_fetch_contract_snapshot()` 保留为单合约入口
- [x] `IB_OPTION_QUOTE_BATCH_SIZE=40`（留余量给 news / price / intraday-spot，它们共享账户线路预算）
- [x] `IB_OPTION_BATCH_WAIT_SECONDS=6` 是**每批**预算而非每合约——批内并行，耗时取决于最慢的那个而非总和
- [x] 保住三条既有不变量：**订阅必然被取消**（含异常路径，泄漏一条线路后续批次就少一条且不报错）、
      **socket 断开秒级失败**、**逐到期日失败隔离**
- [x] 未在窗口内完成的合约**仍产出快照**（用已到的 tick 构建），与串行超时时行为一致；
      illiquid 行权价拿不全是常态，丢弃会静默收窄链而不是如实披露缺口
- [x] 8 个新测试，含"整批订阅完才开始等待"与"40 个合约不得花 40 倍等待时间"

### 连带调整

- [x] **DTE 76 → 150 天**。真正的上限从来不是 `OPTION_MAX_DTE`，是**桶列表**——末桶止于 90，
      而多数标的在 61-90 内只有一两个月度到期日，所以实际停在 76 天（实测 75-90 区间仅 10 个合约 / 1 个标的）。
      单改 `OPTION_MAX_DTE` 无任何效果。新增 `91-120,121-150` 两桶，`OPTION_MAX_CONTRACTS` 120 → 180。
- [x] **quote watchlist 100 → 250**（实际选中 188，即全部够格标的）。
      此前注释断言"再往上需要按 client id 并行"——**那个结论建立在串行前提上，方向是错的**：
      瓶颈不是 client id 数量，是每个 client id 只用了 100 条线路中的 1 条。按 client id 并行只是把低效循环复制几份。

### 我在此过程中的两个错误（记录以免重复）

1. 一度把 `select_quote_watchlist` 的排序键判为 bug 并动手改。实际 `76a700c` 是有意修正且理由更强：
   `total_oi` 来自我们自己存储的链，而链有合约上限，实测 META 存 41,482 对真实 847,623（**20.4 倍**），
   按它排序会把 TSLA/AMD/META 挤到第 54/91/107 位。成交额来自价格数据，不受存储上限影响。已撤回误加的注释。
   **真问题只有一个**：dry-run 把成交额标成 `oi=`，正是它误导了我——已改为 `$vol=`。
2. 一度把 watchlist 设成 150，**没有先核对配置注释里已有的吞吐实测**（164.6s/标的 → 单轮上限 142）。
   已先回退到 95，批量落地后才提到 250。


## 2026-08-15 — 广度：发布时延实测 + 两年历史回填（进行中）

**起因**：`quantrift_stock` 一份 DESIGN 提出把本表当作向前数据源。借此把两件本该早就查清的事查了：
这张表的**可用时刻**，以及**能回填到多早**。本条只做本 repo 的事。

### 结论一：本表落后市场**一整个交易日**，不是几小时

`MARKET_BREADTH_EOD_SETTLE_HOUR_ET=20` 假设当晚就能取到当日数据。实测不成立：

```
08-14 20:05 ET  请求 08-14 → 403，settled on 08-13
08-14 22:05 ET  请求 08-14 → 403，settled on 08-13
08-15 17:10 ET  手动探针：08-14 → 200（12,424 行）
```

两次晚间 cron 都拿不到当日，写入的是前一日。**所以任一交易时段内可读的是 D−2，不是 D−1。**
2026-08-09 加的回退逻辑没问题，问题在于此前只写了「每日自动落库」而没说清是**哪一日**。

- [x] **加早晨探针**（不改代码）：cron `5 17,19` → `5 3,17,19 * * 1-5` PT，即 06:05/20:05/22:05 ET。
      `expected_market_date` 本就把 settle hour 之前的时刻解析为前一交易日，所以早晨这一发天然请求 D−1。
      Polygon 若已隔夜发布，行在 09:30 开盘前落库；若未发布，回退循环幂等重写 D−2，无副作用。
      **日志可区分**：早晨的 `settled on` 若不带 `skipping` 子句，说明 D−1 当时可得。
      发布时刻夹在「D 日 22:05 ET」与「D+1 17:10 ET」之间，周六无法再收窄，交给实测。

### 结论二：grouped daily 是**滚动两年窗口**

```
2024-08-16  200      ← 恰好是今天减两年
2024-02-16  403      2023-08-16  403      2022-08-16  403
2021-01-04  403      2020-01-02  403      2018-05-01  403
```

探测时必须拉开间隔——首轮密集请求返回的是 429，和 403 的权限边界完全不是一回事，混淆会得出错误结论。
下界随日历滚动，**不能硬编码后就信任**，故留 `BREADTH_BACKFILL_START` 且常量注明测定日期。

### 为什么要回填（本 repo 自己的理由）

不是为那份 DESIGN，是为 **Rebound Monitor Phase 1 的 exit gate**：
「统计随后 5/10/20 个交易日收益、最大有利/不利变动、再破低率和 MA20 保持率；walk-forward/样本外切分并披露基准发生率」。
**11 个交易日撑不起这条。** 约 508 个交易日可以做第一轮，但两年内**独立**反弹事件数量有限，
这个局限必须随结果一起披露。

- [ ] **回填进行中**：`collector/backfill_market_breadth.py`，约 2.3 分/交易日、508 个交易日 ≈ 19.5 小时。
      **可续跑**（跳过已有日期 + 写入按 `market_date` 幂等）；**从新到旧**（中断时留下的是与现有数据相连的连续段，而非孤岛）；
      **沿用 provider 默认限速**——`breadth` pacing scope 是我们自己的记账，Polygon 按账户计量，
      回填跑过头会让 option/GEX 通道一起吃 429：**回填慢是成本，生产刷新被限流是事故**。
      不建假日表：休市日没有 grouped 响应，采集器的回退循环会如实报告，另建日历等于给同一事实造第二个真相源。

### Databento 导出**刻意不并入本表**

`~/Downloads/XNAS-20260810-JLNA94QEHN`：`XNAS.ITCH` / `ohlcv-1d` / ALL_SYMBOLS，
2018-05-01 → 2026-08-07，2,079 个交易日，503 MB。八年 vs 我们两年、已在本地、且确实覆盖 NYSE 上市标的
（凡在 Nasdaq 成交过的都在里面）。**但它不能进 `market_breadth_daily`**：

| | 本表 | 该导出 |
| --- | --- | --- |
| 价格 | Polygon grouped daily，**合并整合价** | XNAS.ITCH，**单一场内** |
| universe | XNAS/XNYS/XASE 的 point-in-time 普通股 | 在 Nasdaq 成交过的全部证券，再按目录筛 |
| 成交量 | 全市场 | **仅 Nasdaq 一份** |

同一只 NYSE 股票在该导出里的涨跌由它的 **Nasdaq 收盘价**判定，不是合并收盘价；多数日子结论一致，
但仍是两个不同的量，而成交量口径差得更远。把它写进一张按 Polygon 口径定义、且**每天仍由该口径追加**的表，
会造出一条在拼接处自己和自己不可比的序列。若将来两年样本确实不够，正确形态是**另建一张标注清楚出处的表并列报告**，不是拼接。

详见 `docs/validation/BREADTH_HISTORY_AND_TIMING_2026-08-15.md`。
...
- [x] `collector/capture_ledger_far_leg_marks.py`（阶段一，零 provider 调用，只读当日已落库快照）。
      ...
      **取不到的腿也写行并逐个具名告警**——不写的话「找过但没有」与「从未找过」不可区分。
      PM2 `quantrift-ledger-far-leg-marks`（首版排 13:15 PT；接入 IB 第二趟后改为 12:45 PT，见下）。
...
- [x] **端到端验收通过（终态，08-14 日线补齐至 312/312 之后）**：
      已评分行 **15 → 97**（39 win / 58 loss）、`no_price` **90 → 8**（全部
      `far_leg_mark_missing`，即那 7 个窗口外的深度实值远腿）、已过期未结算 **0**（积压清空）。
      修复过程中的中间态 `evaluateLedger -> {resolved: 0, deferred: 75}` 本身就是证明：
      同样的输入，修复前会写 75 行 `no_price` 并永久定格。
      - 08-14 那批 79 行：多到期日 25 win / 31 loss / 8 no_price，单到期日 5 win / 10 loss。
        **56 行多到期日结构真的结算出来了**，前一天它们还属于「结构上永远不可评」
      - **按族构成整个反转**：time_spread **56 笔**（44.6%）> single_leg 25（24.0%）
        > iron 9（33.3%）> credit_vertical 5（100%）> straddle_strangle 2（0%）。
        原本 0% 可测的族现在是最大的已评分族——
        **台账第一次真的在测量引擎推荐最多的那个东西**
      - ⚠️ **这些胜率一个都不能当结论**：97 笔里 56 笔来自 08-14 同一到期日、
        25 笔来自 07-29 同一天建仓，独立观测数远不到 97；100% 那格只有 5 笔、0% 那格 2 笔。
        能做样本外切分的批次在 09-18（123 笔）与 10-16（129 笔）
...
**零 bid/ask** 的——按该备忘录自己的规则会被自己拦下。追下去不是备忘录的问题,是选取逻辑的问题。

**根因**:`select_quote_watchlist` 按 `total_oi` 排序,但 `total_oi` 是**存下来的 120 个合约**里的 OI,
受 `OPTION_MAX_CONTRACTS=120` 与 `OPTION_MAX_STRIKES_PER_SIDE=6`(约 ±5%)限制。
它测的是**我们的存储窗口,不是市场**。而且偏差是**系统性的**:高价股行权价间距大、OI 落在窗口外,
低价密集行权价的 ETF 则全在窗口内。

对照未被截断的 `oi_by_strike` 宽图实测(2026-08-13):

| 标的 | 窄链测得 | 真实 | 低估 | 原排名 |
| --- | --- | --- | --- | --- |
| META | 41,482 | **847,623** | 20.4× | 107 |
| AMD | 52,453 | **749,994** | 14.3× | 91 |
| TSLA | 110,815 | **1,281,359** | 11.6× | 54 |

全市场期权最活跃的三个名字,被成交额只有其十分之一的标的挤出 50 名清单。

**修复**:排序键改为 `underlying_dollar_volume`——来自价格数据,**任何存储上限都污染不了它**;
`MIN_TOTAL_OI` 仍作为准入门槛把关期权流动性,只是不再决定顺序。

- [x] 排序键修正 + 回归测试锁定(防止有人改回 `total_oi`)
- [x] `QUOTE_WATCHLIST_TARGET` 50 → **100**:实测串行中位 164.6s/标的,
      390 分钟时段上限约 142,取 100 留出超时与重试余量
- [x] 验证:备忘录 8 个标的全部入选(META 14 / AMD 15 / TSLA 13 / DELL 31 / LITE 11 / SNDK 8),
      COHR 也进入(24)。collector 420/420、server 305/305
- [ ] **要覆盖全宇宙(327)需并行化报价通道**(多 clientId + 多 worker),
      而不是继续调大这个数字——串行一天跑不完一轮
...
- [x] **已接入产品(2026-08-13,产品负责人决定)**:`/api/market/positioning` 与 `/positioning`
      新增借券费率列。**这解除了 `ib_internal` 在该字段上的披露边界**。
      页面仍不出现任何数据源名称(只显示数字),符合"不向未认证客户端暴露 provider 名"的既有规则。
      措辞仍受测试约束:仅 >3% 出现提示、>10% 才引用具体数值——全宇宙中位数约 0.34%,
      否则普通标的都会带上一条暗示异常的提示。
...
- [x] **已上生产并加入导航（2026-08-11，产品负责人决定）**：`GET /api/market/positioning`
      （`server/src/domain/market/positioning.js` 纯函数 + DTO）、前端 `/positioning`、导航项「持仓异动」。
      **原「仅后端、不加产品路由」的边界已被明确解除**，理由是当前无用户。
      但措辞边界仍然生效且由测试锁定：DTO 恒带 `calibrated: false`，
      `positioning.test.js` 断言任何 note 不含买/卖/目标/止损等指示性词汇，
      也不含「做市商/被迫/dealer」——公开持仓量无法识别多空双方各自是谁。
      页面正文明示「读数尚未校准、记录自 2026-08-11 起累积」。
      `positioning.js` **完全不读 `gamma_regime`**（符号可能反向）。
- [ ] **积累样本后再谈阈值**；在能做样本外切分之前不得发布统计结论
- [ ] **有真实用户之前需重新评估**：当前页面依赖 IB/TT 阶段性授权豁免，
      且阈值未经校准。认证/订阅落地时两者都要重新过一遍
...
0. **近期生产 bug 修复（多为已完成 ✅，按日期）**：
   ...
   - `2026-07-24 — Watchlist 加个股批量`(两轮请求)✅:
   ...
   - `2026-07-25 — Watchlist 用户大批量重排 + 清洗` ✅：用户一次性把 watchlist 改成 180 个(结构大改,非追加),要求"共同基金和加密货币去掉"并加入 IBIT/ETHU。
     ...
     - ✅ **135 个"被移除"标的实为误覆盖,已全部恢复**:发现 watchlist 从 205→180 不是"净增",而是**新列表覆盖旧文件**,静默删掉 135 个——含**全套 16 个 SPDR 板块 ETF**(R1.3 板块轮动的 `SECTOR_ETFS` 硬编码依赖)、24 个主题 ETF、12 个半导体/科技股、11 个大盘蓝筹。用户确认是意外(先前坚称"CRCL 在文件里",查证确实被覆盖)。处理:取新旧并集、排序去重 → **293 个**,并保留此前 live 验证过的排除项(2 个共同基金、22 个零期权、`BRK-B` 格式错、裸 `BRK`)。验证 R1.3 所需 26 个 ETF **26/26 在位且 scan_enabled**。`scan_enabled` 301 vs watchlist 293 现已基本一致,原"2 倍工作量"问题自然消失,无需批量禁用。
       - **方法教训**:第一次用 `comm` 算 diff 得出"AMD 被删"的**错误结论**,被用户当场纠正——`comm` 依赖两侧排序规则一致,shell locale 会影响结果;改用 Python set 比对才拿到正确答案。
     - 🐛 **回填并行化 + 发现第二个真 bug(429 无重试)**:用户问"要多久/能否加速"。实测串行 **4.02 分钟/标的**,120 个 ≈ **7.3 小时**。瓶颈分析:`backfill_iv_history.py` 是**旁路脚本**,不走主采集器的 `provider_rate_limits` 共享限速闸门,`_get` 裸调用无 pacer 无重试,纯串行 HTTP 往返(每标的约 1500 请求)。评估三条路:①Polygon options **无 grouped 端点**(实测 400,仅支持 stocks/fx/crypto)→ 无算法捷径;②**并行**(采用,标的间完全独立);③合约级区间缓存(可再省约 6 倍请求,需重构逐日流程,**留作后续**)。**6 路并行 20 秒崩 2 个 worker**——"付费档无限调用"仅指无月度配额,**每秒速率限制仍在**,且 429 从 `underlying_closes`(逐日 try/except **之外**)抛出,**直接杀掉整个标的**。修复:`_get` 加 429/5xx 指数退避(尊重 `Retry-After`),单测 +5,collector **277/277**;改用**保守 3 路并行**(给退避留余量),实测零崩溃,预计 ~2.5 小时。可复现:`docs/validation/IV_BACKFILL_PARALLEL_2026-07-25.md`。
       - **结果核查纪律**(承接同日 `occ_ticker` 教训):只认 `computed`/`written` 非零,不看退出码。两种零值含义不同:`days:0` = **确实无数据**;`days>0 但 computed:0` = **需诊断**(既可能是 bug,也可能是该标的压根没有期权可反解——查期权合约数再下结论)(如 `ACAC` 已退市 SPAC,窗口内无价格历史)。
...
- [ ] **P2.1 ⭐盘中用 IB 当 underlying 现价源(代码已完成 2026-07-23,flag 默认关,待开盘 live 验收)**:Polygon Options 档股票盘中数据 403 已铁证死路,IB Gateway 盘中能给真标的价(实测 TSLA 07-20 10:22 ET `ib_internal` underlying=374.43)。设计:常规交易时段 underlying 现价走 IB(Polygon 仍供期权链/GEX);盘后仍用 Polygon `/prev`。
  - **已实现**:复用现成 `IbOptionChainProvider.fetch_underlying`(fallback 路径已在用),worker 加 `fetch_ib_intraday_spot`(best-effort,IB down/晚/受限一律返回 None 不炸 Polygon 路径)+ `is_regular_us_session`(Mon-Fri 09:30-16:00 ET 门控);polygon provider `fetch_option_chain`/`fetch_underlying` 加 `intraday_spot` 参数,优先级最高(IB 盘中价 > Polygon 盘中 gated > spot_hint > `/prev`)。快照 `source` 仍 `polygon_licensed`(期权是 Polygon),真实现价来源记入 `raw_metadata.underlying_source/endpoint/as_of`(与 P3 诚实标注一致)。env `OPTION_IB_INTRADAY_SPOT_ENABLED` **默认 false**——翻开即生效,零改动风险。单测 7 条(provider intraday_spot 胜出+记 provenance/无效回退;worker session 边界/best-effort None/非正价拒绝/默认关)。collector 270/270。
  - **⏳ 待做(仅剩外部门槛)**:美股**开盘时段** flip flag 做 live 验收——确认 `underlying_price` 真取到 IB 实时价、`raw_metadata.underlying_endpoint='ib_intraday_last'`、盘后自动回落 `/prev`;并压测 IB market-data 线程占用(逐 symbol 串行请求即释放,理论在 ~100 并发线内)。市场闭市期间无法验收。可复现:`docs/validation/IB_INTRADAY_SPOT_2026-07-23.md`。**这是当前唯一不花钱拿到真盘中现价的路径**。
...
- [x] **P4 日线 cron 可靠性(已完成 2026-07-23)**:根因是**单次运行 + 采集时点太早**——`quantrift-options-prices` 只在工作日 13:35 PT(=16:35 ET,收盘后仅 35 分钟)跑一次,此时 Polygon 当日 EOD 日线聚合常还没 finalize,于是那根 bar 要等下个工作日才补(周五 07-17 缺到周一)。修复两手:①**加第二次运行** `cron_restart: '35 13,18 * * 1-5'`——18:35 PT(=21:35 ET,过 EOD settle)再跑一次,当日 finalize 的 bar 同日补上;每次运行都重取 400 天并 upsert,第二次运行天然自愈第一次的缺口。②**加新鲜度守卫** `check_price_freshness`:纯函数 `settled_market_date`(按 ET settle 小时判定"该有哪根 bar",周末回退到周五;holiday 未建模→顶多善意 WARNING)+ `symbols_behind`,运行末尾对比 DB 最新 bar 日期,落后就 WARNING(只观测、绝不 fail run),把原来"静默到下周一"的缺口变成可见告警。单测 7 条(settle 判定 4 + symbols_behind + freshness 告警/全新鲜 2)。验证:collector 254/254。可复现:`docs/validation/DAILY_PRICE_CRON_RELIABILITY_2026-07-23.md`。
...
- [x] **R1.2 每日市场简报(对标 nextpick briefing;前后端均完成 2026-07-23)**:市场级 synthesis 一句话综述 + "今日要点"。**增强(2026-07-24 竞品复查)**:headline 挂板块 **S–D 评级**(能源·S、医疗·A 领跑)。
  - **后端** `GET /api/market/briefing`:抽出 `loadBreadth`/`loadStateMatrix`/`loadSectorRotation`(简报与原端点共用一份 query+build,零 SQL 重复,三个 handler 变薄壳)。纯 `buildBriefing`:由 S1 vs S5 定市场倾向(偏多/偏空/均衡),合成 headline(正 Gamma %、IV Rank 中位、状态分布、板块领跑/落后、未来一周财报)+ 结构化 callouts(指数 gamma、财报、异动)。市场级综述、非买卖信号。
  - **数据要素(两处此前我误判、已纠正)**:①**财报**在 `iv_history.earnings_date`(取每标的最新已知、筛未来 7 天,live 有 10 只:KLAC/STX/BA/MSFT/META/AMZN/AAPL 等);②**期权异动**用 `option_oi_delta_snapshots`——**并非"多数为 0"**,近 2 天 15.8 万行非零、confirmed 18.4 万,按 `SUM(ABS(oi_delta))` 聚合 top:NFLX 94.5万/SPCX/XLE/SPY/GDX…。
  - **前端**:`components/MarketBriefing.jsx` 挂在 `/market` 页最顶(headline 卡 + 财报/异动 chip,点→Analyze)。左色条按倾向(多绿/空红)。headline 服务端生成(为后续"每日物化+分享"留口)。
  - **验证**:server 203/203(纯 `buildBriefing` 4 条:倾向+headline/callouts/偏空/空聚合优雅降级)、frontend 92/92 + lint + build 干净。**live headline**:"2026-07-24 市场偏多头,51.3% 标的处正 Gamma,IV Rank 中位 58,状态 强势上行 20/回调 21/空头 9,11 只高波动观望,板块 能源、保险 领跑、太阳能、机器人/AI 落后,未来一周 10 只财报…"。可复现:`docs/validation/MARKET_BRIEFING_2026-07-23.md`。
  - **MVP 范围/待做**:当前**按需计算**(read-only、复用聚合、便宜),尚未"每日物化一份 + 可分享链接/图卡"——物化+分享作为后续步骤。
- [x] **R1.3 板块轮动视图(对标 RRG;前后端均完成 2026-07-23)**:简版 RRG——26 板块/主题 ETF 相对 SPY 的强弱×动量四象限。**增强(2026-07-24 竞品复查,对标 nextpick Institutional Net Flow)**:复用 `deriveMfi` 加**资金流维度**(每 ETF 的 `mfi` → `flow` 流入/流出/中性)+ **S–D `grade`**(from rs);散点 tooltip + 列表 chip 展示(grade 徽章 + 流向三角)。位置=趋势、flow=钱是否真进,二者可背离(实测 KIE/XLV 是 A 级但 MFI 流出)。纯 `rotationGrade`/`rotationFlow` 单测覆盖。
...
- [x] **R3.2 新闻摄取 MVP(对标 newshock,叙事层;定源+实现均完成 2026-07-26)**:把 `synthesis.js::volatilityAttribution` 里的免责声明从"隔夜跳空"粒度升级到具体 headline(有真实新闻时);Analyze 页新增"近期消息"区块(headline+来源+时间)。**暂无跳转链接**——IB `tickNews` 只给 headline/provider/article_id,不给文章 URL,取全文/链接要靠 `reqNewsArticle`(已为它预留 `provider_code`/`article_id` 字段,但本次 MVP 未实现,算作 v2 范围)。**MVP 不做 LLM 分类**,先上 $0 版验证参与度,v2 再加 Claude Haiku 事件分类(约 $5-30/月,已估算)。
  ...
  - **写完后 live 实测发现 `reqHistoricalNews` 返回 0 条(同日,已修复)**:5 个真实标的(AAPL/TSLA/NVDA/MU/SPY),48h 和 96h 窗口都拿到 0 条。逐步排查排除了"周末新闻荒""`totalResults` 截断""conId/事件解析 bug"——最终定位:同一查询间隔几分钟重跑,"最新一条"时间戳会变得更旧(比如从 07-24 15:30 退回到 07-22 03:05)。查 IB 官方文档([news.html](https://interactivebrokers.github.io/tws-api/news.html))确认 `reqHistoricalNews` 本来就是查"**系统缓存**里的历史新闻"("cached in the system"),没有任何刷新 SLA 文档——不是我们代码的 bug,是这个接口本身不保证新鲜度。**改用 IB 另一套机制:`reqMktData`+genericTick `292`(`tickNews` 回调)——实时推送,不查缓存**。live 对比:同一批标的,`reqHistoricalNews` 最新一条 4 天前;`tickNews` 最新一条 **49 分钟前**。已重写 `IBNewsProvider.fetch_recent_news()`,按 IB 行情线路上限(默认 100/账户)分批订阅(批大小 80),每批开 20 秒抓突发推送再退订。生产方法 live 实测通过:5 标的 20 秒内拿到 8 条真实新闻(此前 0 条)。**副作用**:分批并行订阅比原来逐标的顺序请求快得多——292 个标的全量扫一遍从预计 39 分钟降到约 **80 秒**(4 批 × 20 秒),节奏可能从"每小时"收紧到"每 5-10 分钟",但本次未重新拍板,先维持每小时。详见 `docs/validation/NEWS_SOURCE_SELECTION_2026-07-26.md`。
  - **合规边界(不变)**:只做客观摘要+来源链接,不做"AI 荐股"式文案、不包装成买卖信号、不编造无来源支撑的解读。
  - **采集节奏拍板(2026-07-26,定时任务,非常驻)**:常驻订阅可以做到秒级实时,但两个原因决定先不做:①**Mac/IB Gateway 本来就要 7×24 开着**(`quantrift-options-collector`/`ib-market-data-fetcher` 等已连续跑数天),这点在两个方案里都一样,不是常驻独有的成本;②MVP 阶段是验证"有没有人看这个板块",展示型的"近期消息"滞后几分钟完全够用,常驻的秒级价值只有做实时提醒/推送时才兑现。**定为每 5 分钟跑一次**(`cron_restart: '*/5 * * * *'`,PM2 `autorestart:false`,同 `collect_reddit_trends.py` 的模式)——全量扫一遍约 80 秒,5 分钟窗口留足余量不会重叠。项目里已有一个功能对等的常驻先例(`quantrift-unusual-whales-flow`,WebSocket 流式,已稳跑 9 天),升级路径留着,不是重新架构。
  - **实现完成 + live 验收(2026-07-26)**:`GET /api/news/:symbol`(`server/src/routes/news.js`,累积日志型,查无当作正常"安静"状态而非"缺失/过期",不套用单快照类产品的 freshness 契约)、Analyze Tab3「近期消息」卡片(`az-news-*`,前端按 headline 文本去重同一真实新闻的多个 provider 变体)、`synthesis.js::volatilityAttribution` 新增 `recentHeadlines` 参数(有真实新闻时替换隔夜跳空占位文案,附带来源标注和"仅参考不构成因果"措辞)、`collect_news.py` 接入 `ecosystem.config.cjs`(`quantrift-news`,每 5 分钟)。**真实运行验收**:对生产 300 标的全量跑一次,`fetched:49, written:49, symbols_with_news:12`;`GET /api/news/AAPL` 返回真实 headline(如 18:00 发布的 Barron's 报道),服务器/前端测试全绿(server 225/225、frontend 95/95+lint+build、collector 284/284)。**未验证**:未能在浏览器里目视确认 Analyze 页面渲染(本次会话浏览器自动化被拒绝),已通过 curl 确认 API 契约正确、前端 reducer/组件单测覆盖了渲染前的数据形状,但没有肉眼看过最终页面——建议下次有浏览器权限时补一次目视检查。
...
- 🟡 **前端切流（E17，2026-07-17 代码完成；2026-08-09 已合入 master，仍待人工浏览器验收）**：
  原分支 `feat/v3a-frontend-cutover`。合并时该分支落后 master 132 个提交，`Analyze.jsx` 有 4 处冲突，
  **全部按并集解决**——master 期间新增的 news（`applyNews`/`getNews`）与 technical levels
  （`getTechnicalLevels`/`technicalResult`）与本分支新增的 `applySummary`/`getAnalyzeSummary` 同时保留，
  `Promise.all` 数组顺序与解构一一对应（第 13 位 technicalResult、第 14 位 summaryData）。任一侧被丢弃都会静默移除一个已上线功能。
