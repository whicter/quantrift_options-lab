...
- 2026-07-16 authentication incident: a successful TT session exchange can return a successor `remember-token`. The previous collector discarded that successor, leaving the next run with an invalid old token. `provider_auth_state` in PostgreSQL is the durable source of truth **for collectors bound to the same `DATABASE_URL`**: `auth.py --login` stores a fresh seed there, and every collector obtains a transaction-scoped advisory lock, exchanges the current token once, then commits the provider-supplied successor (or the unchanged token when none is returned). Railway needs both `TT_LOGIN` and `TT_REMEMBER_TOKEN`; missing `TT_LOGIN` fails before any HTTP request. Railway uses `TT_REMEMBER_TOKEN` only to bootstrap its own state row when that row is absent. Railway cron needs no Volume and no per-run successor-variable update. A 401/403 never changes the database row and never retries credentials.
