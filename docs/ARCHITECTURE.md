...
Analyze 并行读取 metrics、daily prices、GEX、unusual、S/R、Volume Profile 和 chain stats。S/R/Focus 只读取 `price_history`；Volume Profile 只读取 regular-session `price_history_30m`，默认近 20 天、40 个价格桶；IV skew/term structure 只读取 `option_contract_snapshots` 中实际存在且 `iv > 0` 的合约。PostgreSQL `DATE` 在 API 边界统一序列化为 `YYYY-MM-DD`，DTE/当日完整性使用 `America/New_York`。
