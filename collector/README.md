...
- 2026-07-16 first cloud run reached PostgreSQL and loaded 67 symbols, then stopped at TT remember-token renewal (`401 invalid_credentials`). A local no-write probe of the currently configured token returned `403`; replace `TT_REMEMBER_TOKEN` after `python auth.py --login` and manually rerun before treating cloud collection as live.
