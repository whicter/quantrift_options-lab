...
- Known architecture issue (2026-07-17, unresolved — see task.md 2026-07-17): the option refresh cycle runs in TWO places against one DB — the Mac Studio PM2 daemon (`run_collector_daemon.py`, loop) AND a Railway cron (`run_railway_refresh_cycle.py` via `collector/railway.metrics.json`, every 5 min). No single writer, so the two runtimes contend on shared state (e.g. the `provider_request_usage` budget row, which each stamps with its own `PROVIDER_DAILY_BUDGET`); the dedupe/`NOT EXISTS`/stale-recovery guards exist to paper over the double-run. Only IB Gateway and TT-authenticated metrics are genuinely local-constrained; everything Polygon-based is cloud-friendly. Target: one owner for the Polygon pipeline (either disable the Railway cron so Mac Studio is sole writer, or move the pipeline to Railway and leave Mac Studio as an IB/TT-only adapter). Until resolved, any `PROVIDER_DAILY_BUDGET` change must be applied to BOTH runtimes or the lower one stomps the shared budget row.
