...
- 2026-07-16 authentication incident: a successful TT session exchange can return a successor `remember-token`. The previous collector discarded that successor, leaving the next run with an invalid old token. `provider_auth_state` in PostgreSQL is now the shared source of truth: `auth.py --login` stores a fresh seed there, and every collector obtains a transaction-scoped advisory lock, exchanges the current token once, then commits the provider-supplied successor (or the unchanged token when none is returned). The additive migration was run successfully and the table was read-only verified empty before the new login. Railway cron needs no Volume and no per-run token-variable update. A 401/403 never changes the database row and never retries credentials. The current token was consumed by the pre-fix probe; run `python auth.py --login` once more, then run `collect.py` directly without a separate session probe.
