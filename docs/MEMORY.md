...
Tastytrade metrics cron is deployed as Railway service `quantrift-metrics-cron`: one-shot image/config runs weekdays at 22:30 UTC and exits. The 2026-07-16 failed run connected to PostgreSQL and loaded 67 symbols, then TT rejected the stored Railway token. If that state differs from the configured `TT_REMEMBER_TOKEN`, the collector now tries the configured seed once after an explicit 401/403 and persists it only on success. Cloud-run completion remains unconfirmed until its execution log and the resulting `iv_history`/`provider_auth_state.updated_at` are verified.
