...
flowchart TB
    ...
    SPA -->|GET /api/technical-levels/:symbol| Express
...
```env
VITE_API_URL=https://quantriftoptions-lab-production.up.railway.app
```

Technical Levels client 同时兼容旧配置名 `VITE_API_BASE_URL`，新部署统一使用 `VITE_API_URL`。
...
sequenceDiagram
    ...
    User->>UI: 输入 AAPL
    UI->>UI: 标准化并校验 symbol
    UI->>API: GET /api/technical-levels/AAPL
    API->>DB: 查询 250 日线、30m OHLCV、GEX/OI 快照
    DB-->>API: 价格与期权快照
    API->>API: MA/ATR/Volume Profile/AVWAP/周线/Confluence
    API-->>UI: JSON
    UI-->>User: S/R 区域、证据、强度与期权数据状态
```
...
| GET | `/api/technical-levels/:symbol` | 计算股票技术支撑/压力共振结构 |
...
`/api/technical-levels/:symbol`：

- symbol 统一大写，只允许 1–12 位字母、数字、`.` 和 `-`。
- 日线不足时返回 `missing`，不生成技术位。
- GEX/OI 查询失败或快照缺失时，技术结构仍可返回；期权子结构明确标记 `missing`。
- Volume Profile 基于常规交易时段 30m bar typical price，是 bar-level approximation，不声明为逐笔成交分布。

### 7.5 Technical Support Confluence

```text
price_history (250 daily)
  → 50/100/200DMA + ATR14 + daily pivots + weekly aggregation

price_history_30m (regular session)
  → Volume Profile POC/HVN + Anchored VWAP

gex_snapshots + option_contract_snapshots
  → GEX walls + 7–60 DTE OI walls

all evidence
  → split by spot into support/resistance
  → cluster within max(0.5 × ATR14, 0.5% × spot)
  → ranked zones with evidence and source metadata
```

OI Wall 与 GEX Wall 是不同统计口径，API 和 UI 均保持独立。期权快照不是技术结构的硬依赖。
...
- Technical Levels payload 标准化、missing/stale/error 状态。
...
- 数据库异常映射。
- SQL 注入输入。
- Technical Levels 的 DMA、ATR、Volume Profile、AVWAP、周线、聚类与 Options missing/ready。
- CORS。
- 最大 limit。
