  ...
  return (
    <>
      ...
      <section className="account-plans">
        ...
        <div><h2>Pro</h2><p>更及时的市场数据、期权扫描、条件提醒与持仓记录。数据频率和可用范围可能变化。</p>{account.subscription.plan === 'pro' ? <button type="button" onClick={() => openBilling('portal')}>管理订阅</button> : <button className="primary-btn" type="button" onClick={() => openBilling('checkout')}>升级 Pro</button>}</div>
      </section>
    </>
  );
}
