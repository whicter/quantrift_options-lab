...
├── docs/              → 架构、部署、Wiki、学习与任务文档事实源
└── README.md / AGENTS.md / CLAUDE.md → 根目录标准入口
```
...
| `GET /api/technical-levels/:symbol` | Volume Profile/AVWAP/多周期/GEX/OI 共振区域（代码完成，待生产部署） |
...
`GET /api/technical-levels/:symbol` 是现有 `/api/sr/:symbol` 的扩展技术结构产品，读取数据库快照，
不在用户请求路径同步调用 provider。计算包括：
...
6. 先按现价分 support/resistance，再以
   `max(0.5 × ATR14, 0.5% × spot)` 聚类为可解释区域。

每个区域返回 `low/high/center`、`score`、`strength`、`distance_pct` 与 evidence。
Volume Profile 明确是 30m bar typical-price approximation，不等同逐笔成交分布；GEX Wall
与 OI Wall 不得混用。

SPY 有价格历史且属于旧 mock 白名单，因此部署后会同时显示真实 Technical Levels 和旧版 4-Tab。
只有价格历史、没有 mock 的 symbol 仍显示技术结构；没有日线时返回 `missing`；没有期权快照时
只将 GEX/OI 标为 `missing`，不会伪造 Wall。

代码位于 `da298f4`。GOOG 生产输入 smoke 得到 POC `346.00`、AVWAP `353.42`、
50/100/200DMA `366.12 / 343.21 / 321.99`。该 smoke 验证计算但不代表上线；仍需 Railway
部署 API、验证 `/api/technical-levels/SPY`，再部署 Vercel 面板。
