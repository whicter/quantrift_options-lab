...
**Directional/gamma weighting (2026-07-24)**: `candidateEngine.cjs::directionalWeight(strategy, environment)` weights a candidate's score by market context — trend alignment (`STRATEGY_STANCE`), IV rank (buy vs sell premium), and dealer gamma regime (`STRATEGY_GAMMA_PROFILE`: `long_gamma`/`short_gamma` per strategy). Negative dealer gamma (hedging amplifies moves) boosts `long_gamma` strategies and discounts `short_gamma` ones; positive gamma does the reverse — an independent, informational `gammaNote` (never a `conflict`, since it's a soft market-structure tilt, not a directional call the strategy is "wrong" about). The guard is on `environment` itself, not `environment.trendRegime`, specifically so a gamma-only environment still applies: **the whole subsystem (trend + IV + now gamma) went unused in production until this change** — both real callers (`scan.js`, `materializeScannerCandidates.js`) always called `buildActionableSetups` without an `environment` argument, so `directionalWeight` silently no-opped since it was written; `row.gamma_regime` was on hand but never threaded through. Both call sites now pass `{ gammaRegime: row.gamma_regime || null }`. `trendRegime` remains unwired (no real-time trend source is constructed at either call site yet). Repro: `docs/validation/GAMMA_REGIME_STRATEGY_WEIGHT_2026-07-24.md`.
...
**Interleaving, not a global sort (2026-08-15)**: strategies differ ~16x in how many combinations they enumerate (Diagonal Spread 6,761 of 11,116 candidates on batch 2056, against Long Put's 414), so pooling everything and sorting by score ranks by enumeration size — drawing 16x more samples from the same distribution wins the top slots far more often, a multiple-comparisons artifact rather than evidence of quality. `interleaveByStrategy` was written for exactly this, unit-tested, and correctly called at the end of `buildActionableSetups` — and `buildCandidateBatch` then re-sorted the pooled result and undid it, so the path feeding `/api/v1/scanner/candidates` **and** `candidate_ledger` had no diversity guarantee at all: the top 20 of the live batch were Diagonal Spreads without exception, across two symbols, and that family is precisely the one the ledger could least validate. That sort also read the raw `score` rather than `effectiveScore`, discarding `directionalWeight`'s output after computing it — the second time this subsystem has been silently bypassed at a call site (see §Directional/gamma weighting). **A unit test on a pure function cannot show the feature is live**; assert on what the caller finally produces.
...
The candidate batches above are **pruned** (V3A-2 keeps ~5 completed batches per scan_key), so a candidate does not survive to its expiry. To score real outcomes, a **durable** `candidate_ledger` table (`UNIQUE (candidate_key, expiry)`) captures each candidate once at first sighting — its entry (`entry_cash` credit-positive / debit-negative, `max_loss`, `pop`, legs). `captureLedger`/`evaluateLedger` (`server/src/routes/ledger.js`) are wired best-effort into `runMaterialization`, so every scan cycle captures new candidates and resolves any now-expired ones. Resolution uses the pure `server/src/domain/scanner/ledger.cjs::evaluateOutcome`: a single-expiry defined-risk payoff at expiry (`entry_cash + Σ BUY:+intrinsic / SELL:−intrinsic`) from the underlying's `price_history` close on/after expiry; a missing close → `no_price`.

**Multi-expiry settlement (2026-08-13).** Calendars/diagonals used to return `not_evaluable/multi_expiry` unconditionally, which described them as structurally unscoreable when the real situation was that nobody had captured the far leg's closing price. Measured on production that day, **355 of the 538 rows captured since 2026-08-10 were multi-expiry, and the top 20 candidates of the latest batch were Diagonal Spreads without exception** — the ledger could not measure the very candidates it ranked highest. (The same shape was recorded on 2026-07-30 at 64%; the fix applied then, `LEDGER_CAPTURE_TOP_N_PER_SYMBOL=3`, reduced the row count and left the proportion at 60%.) A row now settles on the **earliest expiry among its legs** — derived from `legs_json`, never read off the `expiry` column, so the two cannot disagree about which leg is near. Legs expiring that day settle at intrinsic; surviving legs are closed at an **observed** mark supplied through `evaluateOutcome(entry, close, { farLegMarks })`. The two failure modes are deliberately distinct: omitting `farLegMarks` entirely means the caller never tried and still yields `not_evaluable`, whereas supplied-but-incomplete marks yield `no_price/far_leg_mark_missing` — a data gap better coverage fixes, not a property of the structure. A model price must never stand in for a missing mark; that would make the ledger score the model against itself.

**Writing an outcome is what makes it final** — `evaluateLedger` selects on `outcome IS NULL`, so a resolved row is never revisited. Retryable and terminal gaps must therefore be told apart by *reason*, not by result: a missing daily close is late-arriving data about a past session (settling against it later is not look-ahead), while a missing far-leg mark can only ever be observed on the settlement date. Only the former is deferred, leaving the row unresolved until the bar lands or `LEDGER_CLOSE_GRACE_DAYS` (7) passes; the latter is final immediately. Until 2026-08-15 both were written straight to `no_price`, and because Polygon had published the daily bar for only 49 of 312 symbols that Friday evening, all 79 rows expiring 2026-08-14 were permanently discarded — including 64 whose one-shot marks had been captured correctly that afternoon and were never read, since the close check runs first. `resolution_reason` records which check failed, and `evaluateLedger` returns `{resolved, deferred}` so a run blocked by a lagging price feed cannot read as a run with nothing to settle.

Marks live in the **durable, foreign-key-free** `ledger_far_leg_marks` (`UNIQUE (settlement_date, symbol, expiry, strike, option_right)`), written by `collector/capture_ledger_far_leg_marks.py` on weekdays (PM2 `quantrift-ledger-far-leg-marks`; schedule and the two passes below) and backed up via `backup_facts.TABLES`. The durability argument is stronger than for `gex_history`: each mark is observable on **exactly one date**, so a missed run is permanent — the chain snapshot behind it is pruned within 7 days, and a quote from any later session is a different day's price, which is the look-ahead the ledger exists to prevent. Phase one reads only quotes already persisted for that trading day and takes a mark **solely from a two-sided market** (`bid > 0 AND ask >= bid`); `last` is a transacted price and the snapshot's own `mark` column can be model-derived. Legs it fails to price are still written (`mark` NULL, `source='missing'`) and named individually in the log, because an unrecorded miss is indistinguishable from a leg nobody attempted and makes coverage unmeasurable; `mark IS NULL` is filtered before evaluation so it can never settle a leg at zero. The capture runs **two passes at 12:45 PT (15:45 ET, inside the session)**: the day's persisted snapshots first, then `IbOptionChainProvider.fetch_named_contracts` for whatever is left — an explicit contract list resolved through `reqContractDetails` and matched on `conId`, grouped by `(expiry, right)` so one lookup serves every strike sharing them, never an expiry×strike×right product. Pass 2 is gated on an open session (with no quote stream IB burns its per-contract timeout and returns nothing) and on `settlement_date == today`: quoting today to settle a past date would pass one day's market off as another's close, the exact look-ahead this table exists to prevent. 13:15 PT was the original schedule and is 16:15 ET — after the close, where pass 2 can never run.

Measured 2026-08-14: **41 of 48** far legs from snapshots alone, all `ib_internal`; the 7 misses were deep-ITM stock-replacement legs outside the ±5% strike window, which is what pass 2 exists for — **do not widen `OPTION_MAX_STRIKES_PER_SIDE` for them** (a settlement date needs 4–147 contracts, against the 120–240 one ordinary chain sweep already costs). Settling those rows took the ledger from **15 scored rows to 97**, with the 79 rows expiring that day resolving 25/31 win/loss on the multi-expiry side and 5/10 on the single-expiry side, and 8 remaining `no_price/far_leg_mark_missing`. `time_spread` thereby became the **largest** scored family (56 rows, 44.6%) having previously been 0% scoreable — the ledger measures what the engine recommends most for the first time. Those rates are not conclusions: 56 of the 97 share one expiry and 25 share one entry date, so the first batches supporting an out-of-sample split are 2026-09-18 (123) and 2026-10-16 (129). `aggregateLedger` computes win rate by strategy family and POP calibration over win/loss rows only. The ledger is backend-only: there is no product route, navigation item or public read endpoint. Like the IV-Rank 252-day gate this **starts empty and fills as candidates expire** (earliest 2026-08-21); it is internal validation data, never a trade signal. Repro: `docs/validation/CANDIDATE_LEDGER_2026-07-24.md`.
...
**`option_contract_registry` is a cache, not a fact table (2026-08-29).** It records which option contracts IB lists for a `(symbol, expiry, option_right)` on a given trading day, keyed to the strike and carrying the `conId`. It has no foreign key in either direction: a universe prune must not delete contract listings, and truncating the registry costs one slow sweep rather than any data. Nothing reads it except `providers/option_contract_registry.py`; no product route touches it, and it is deliberately **not** in `backup_facts.TABLES` — it is reconstructible from IB in one pass.

It exists because discovery, not quoting, is the half of a quote job that re-asks a question whose answer did not change: `reqContractDetails` is called twice per expiry (call and put), and IB adds strikes when spot travels far enough to need them, not continuously. Measured on the same 80-strike AAPL ladder (2026-08-28): 2.12s from IB against 0.132s from the registry.

The read is deliberately conservative — a hit is returned **only** when the cached ladder cannot change the selection, meaning it either spans the whole strike window or already supplies the full `max_per_side` complement on both sides of spot. A ladder that runs out on the side spot moved toward is a miss, because that is exactly where a newly listed strike would be. Reads and writes are best effort in both directions: no database, no table, or a stale-looking ladder all degrade to asking IB, which is the behaviour the cache replaced. `refreshed_on` scopes a ladder to one trading day, so the first pass each morning is all misses by design.

Note the trap this measurement fell into first, since it applies to any latency work here: the split between discovery and quoting **cannot be recovered by regression** on job durations, because expiry count, ladder size and kept-contract count all move together across symbols. An earlier fit appeared to separate them at R²=0.95 and was reporting two collinear proxies for one variable — it also assumed the code-default `IB_OPTION_QUOTE_BATCH_SIZE=40` while production runs 90 against `max_contracts_per_expiration=80`, which makes every expiry exactly one batch. `raw_metadata.discovery_timings` and `raw_metadata.batch_timings` now clock each half directly. Repro: `docs/validation/QUOTE_LANE_LATENCY_2026-08-29.md`.
...
Borrow availability (2026-08-13) fills the one input a short-squeeze read needs and no source here carries: cost to borrow. Polygon has none, Ortex and S3 sell it for four figures a year, and IB's API returns availability (generic tick 236 -> tick 46 level, tick 89 shares) but never the fee tick; IBKR's own file server does publish FeeRate but outbound FTP is blocked from this host, which leaves the cheapest route to the fee untested rather than ruled out. Availability substitutes because it is the same scarcity from the other side -- when the lendable pool collapses the fee spikes -- and the usable signal is its **trend**, which is why `borrow_availability_history` is a daily capture rather than an on-demand read. `status` keeps "IB will not lend this" distinct from "IB had no answer": the first is an observation at the extreme of the scale, not a null. Client id 44 keeps it off the option (42), price (12) and news (55) lanes. Borrow cost reaches the product as of 2026-08-13, on an explicit decision: `/api/market/positioning` joins the latest non-null `fee_rate` per symbol (its own most recent date, not the squeeze date, so a missed borrow capture shows the last real reading rather than a hole) and `/positioning` renders it. That lifts the `ib_internal` disclosure boundary for this one field, and the page still names no provider -- only the number, per the rule against exposing source names to unauthenticated clients. Copy stays descriptive: a note appears only above 3% and quotes the figure above 10%, since the universe median is ~0.34% and every ordinary name would otherwise carry a note implying something unusual. Repro: `docs/validation/BORROW_AVAILABILITY_2026-08-13.md`.

**Served publicly since 2026-08-11** at `GET /api/market/positioning` and `/positioning` (nav: 持仓异动), on an explicit product decision that the site has no users yet. The backend-only boundary that applied at capture time is therefore lifted for this view, but the **copy** boundary is not, and it is enforced by tests rather than convention: `server/src/domain/market/positioning.js` is pure, always emits `calibrated: false`, and `server/test/positioning.test.js` asserts that no generated note contains an instruction (买/卖/目标/止损/建仓/入场) or a dealer claim (做市商/被迫/dealer). It also never reads `gamma_regime` — the sign may be inverted for this use — and only `asset_type='stock'` rows are served. The page states on its own face that the readings are uncalibrated and that the series began 2026-08-11. Two things must be revisited before real users arrive: the IB/TT staged licensing exemption this view inherits, and the uncalibrated thresholds.
...
- **可靠性（P4，2026-07-23）**：cron 每工作日跑两次 `35 13,18 * * 1-5`。运行末尾 `check_price_freshness` 用纯函数 `settled_market_date` 对比 DB 最新 bar，落后即 WARNING——只观测、绝不 fail run。可复现：`docs/validation/DAILY_PRICE_CRON_RELIABILITY_2026-07-23.md`。
- **日线走 grouped daily，因为 per-symbol 扫描会按字母表切分 universe（2026-09-17）**。上一条里"18:35 PT（=21:35 ET，过 settle）"这个假设不成立：Polygon 对当日 session 一律回 `403 NOT_AUTHORIZED` —— *"Attempted to request today's data before end of day"* —— 实测该 gate 在 ~00:00 ET 才打开，即**下一个 ET 日历日**。`collect_prices` 在 `POLYGON_STOCK_REQUEST_DELAY=16` 下按字母序跑 2h55m，那一轮正好横跨 gate：切点前抓的标的静默拿不到 bar、切点后的拿得到，结果 319 个标的里只有 52 个（全部 U–Z）是当日的，scanner 80% 的行用着两个 session 前的收盘价。现在日线填充走 `PolygonPriceProvider.fetch_grouped_daily()`——一次请求返回全市场当日 OHLCV，**一个 session 要么对所有标的落地、要么一个都没有**，与标的排在扫描哪个位置无关。per-symbol `fetch_daily_bars` 降级为回补路径，由 `symbols_needing_history()` 挑选：库内无历史、最新 bar 早于 grouped 窗口、或窗口内出现拆股量级的收盘跳变（grouped 的 bar 只按自己那个 session 复权，拆股会把未复权历史接在已复权 bar 上，静默毁掉所有建在上面的均线）。`PRICE_EOD_SETTLE_HOUR_ET` 改为 24——小时只有 0..23，24 即"session 永不在自己日期上可取"。注意：要让 session D 的收盘价在 **D+1 盘中**就位，仍需一轮跑在 00:00 ET 之后，而现有两轮都在之前。可复现：`docs/validation/DAILY_PRICE_LAG_AND_THROUGHPUT_2026-09-17.md`。
...
This backfill is a **bypass path** — it does not go through the `provider_rate_limits` gate that paces the main collector, so `_get` carries its own bounded 429/5xx retry with exponential backoff (honoring `Retry-After`). Polygon's paid plan has no monthly quota but does rate-limit per second, so this matters as soon as the work is parallelized. Because symbols are independent and writes are keyed by (symbol, date), the symbol list can simply be split across processes; 3 concurrent workers is verified-clean and cuts a ~7.3h serial run of 120 symbols to ~2.5h, while 6 workers tripped 429s hard enough to kill two of them within 20 seconds. The OCC root strips punctuation (`BRK.B` → `O:BRKB…`); getting this wrong 404s every request and, because per-day errors are swallowed best-effort, backfills 0 days while still logging "N/N trading days processed" — so completion must be judged by non-zero `computed`/`written` counts, never by the exit code. Two zero-shapes are legitimate and must not be mistaken for that defect: `days: 0` means no underlying price history in the window (e.g. a delisted SPAC), and `days > 0, computed: 0` also occurs when a symbol has prices but no options to invert at all — verify against the underlying's option-contract count before calling it a bug (2026-07-25: 6 such symbols, all legitimate, including `WR`, a recycled ticker whose only historical options belong to the pre-2018 Westar Energy). Repro: `docs/validation/IV_BACKFILL_PARALLEL_2026-07-25.md`.
...
**The gate has since been crossed at scale.** Measured 2026-08-09: **223 of 302** symbols carried `iv_rank_ready=true` on that market date (229 symbols ready at some point, max 297 observations), against the 0/67 recorded when this shipped. Derived IV Rank is now self-sufficient for the large majority of the universe, so the Tastytrade cold-start rank is a fallback for the tail rather than the primary source — which also means the "TT treats Railway as an untrusted device, so Mac Studio is the only metrics writer" constraint no longer gates IV Rank for most symbols. Readiness stays per-symbol and factual; a symbol short of 252 observations still reports `iv_rank_ready=false` and falls back explicitly.
...
`GET /api/market/briefing` (R1.2, 2026-07-23) is the market-level daily briefing: it reuses the extracted `loadBreadth`/`loadStateMatrix`/`loadSectorRotation` helpers (each the single query+build shared with its own endpoint) plus regime gamma, upcoming earnings (`iv_history.earnings_date`, next 7 days) and top option activity (`option_oi_delta_snapshots` `SUM(ABS(oi_delta))`). Pure `buildBriefing` derives a market tilt (S1 vs S5) and a synthesized headline (positive-gamma %, IV-rank median, state distribution, sector leaders/laggards, earnings ahead) plus structured callouts — a market summary, never a trade signal. The headline is server-composed so it can be materialized/shared later; the `/market` page renders it at the top (`components/MarketBriefing.jsx`). Repro: `docs/validation/MARKET_BRIEFING_2026-07-23.md`.

`GET /api/market/earnings-this-week` is the corresponding calendar read model. It selects each active, scan-enabled universe symbol's latest persisted `iv_history.earnings_date`, limited to the current New York Monday–Friday week; `?week=next` shifts the same window forward one calendar week. It joins nullable `symbol_universe.name`. `/earnings` has an in-page 本周/下周 switch while always rendering five weekday columns (including an honest empty state); each row links to `/analyze?symbol=…`. The source has no reliable before/after-market time or estimate, so neither is invented.

Ticker reference metadata may include provider-issued branding `icon_url`/`logo_url`. The collector validates and persists HTTPS branding URLs in `symbol_universe.metadata`; the earnings calendar returns only the stored `branding_icon_url` and renders it when available. There is no browser-side logo lookup. An unavailable image falls back to a ticker initial, never a guessed company logo.

`GET /api/market/sector-rotation` (R1.3, 2026-07-23) is a simplified RRG over 26 curated sector/theme ETFs vs SPY — the SIC `sector` field is 65% null and never classifies ETFs, so each ETF is its own sector proxy (the canonical RRG). Pure `buildSectorRotation`: `rs` = ETF 20-day return − benchmark's; `momentum` = recent relative pace (`ret5 − bench.ret5`) − the month's average per-5-day pace (`rs/4`); the (rs, momentum) sign pair maps to leading/weakening/improving/lagging. It fails closed when the benchmark has no return. Two enhancements from the 2026-07-24 competitor re-audit: a strength **grade** (S–D from rs, cf. nextpick's sector grades) and a money-**flow** read from **MFI** (reusing `supportResistance.deriveMfi` over recent OHLCV, cf. nextpick's "Institutional Net Flow") — position is trend, flow is whether money is actually moving in; the two can diverge (e.g. a grade-A leader with MFI outflow). Grades also appear in the R1.2 briefing headline (能源·S、医疗·A 领跑). The `/market` surface renders it (under the State Matrix) as a scatter + a linked quadrant list — the scatter carries the flow position, the list stays legible at any dot density, and a single hovered state cross-highlights the twin; positions come from the pure `lib/sectorRotation.js::dotPosition`, no chart library. Repro: `docs/validation/SECTOR_ROTATION_2026-07-23.md`.
...
R2.3 (2026-07-30) adds a distinct **full-market EOD breadth** plane; it does not rename the scan universe. `collector/collect_market_breadth.py` uses Polygon Grouped Daily for the latest settled session and its previous real session, then intersects those rows with point-in-time `type=CS` references for primary exchanges `XNAS/XNYS/XASE`. `include_otc=false` and the CS filter exclude OTC/ETF/preferred/warrant rows. The collector computes issue breadth (advance/decline/unchanged, net advances, A/D ratio), volume breadth and per-exchange breakdown, requires at least 2,000 comparable issues and 90% prior-close coverage, and upserts one compact row into `market_breadth_daily`. It is an isolated PM2 one-shot; no option/GEX job and no IB call is involved. `/api/market/breadth.broad_market` reads the latest row plus up to 30 stored sessions and returns a cumulative A/D display series. Until a run passes the quality gate, the API correctly returns `status=missing` without substituting scan-universe data. Repro: `docs/validation/FULL_MARKET_BREADTH_2026-07-30.md`.

~~no row exists yet because the collector environment lacks `POLYGON_API_KEY`~~ — **that attribution was wrong and was corrected on 2026-08-09**: the key was always present. The collector died because `latest_grouped_on_or_before` caught only an *empty* grouped-daily response, not an HTTP error, so the 403 Polygon returns for a not-yet-published session propagated out of the walk-back loop instead of stepping back one day. See `docs/validation/OPTION_QUOTE_COVERAGE_2026-08-09.md`.

**Timing (measured 2026-08-15, `docs/validation/BREADTH_HISTORY_AND_TIMING_2026-08-15.md`).** The row readable during any session is **D-2, not D-1**. Both evening fires take a 403 on the session that just closed and settle on the prior one; a probe the next afternoon gets it fine. So "updated daily" is true and insufficient — a feature defined on `t-1` cannot be computed from this table during `t`. The cron is now `5 3,17,19 * * 1-5` PT (06:05 / 20:05 / 22:05 ET); the morning fire asks for D-1 with no code change because `expected_market_date` already resolves any hour before the settle hour to the previous session, and its log line reports whether D-1 was available. Anything consuming this table must read `market_date` rather than assume it.

**History.** Polygon grouped daily is a **rolling two-year window** (2024-08-16 → 200, 2024-02-16 and older → 403, probed 2026-08-15), so the floor moves with the calendar and must be re-probed rather than trusted. `collector/backfill_market_breadth.py` extends the series over that window for the Rebound Monitor's Phase 1 historical-event validation, which eleven sessions cannot support. It is resumable (skips stored dates, writer is idempotent on `market_date`), runs newest-first so an interrupted run leaves a stretch contiguous with existing data rather than an island, and keeps provider-default pacing — the `breadth` pacing scope is local bookkeeping while Polygon meters the account, so outrunning the shared limiter would earn 429s for the option/GEX lane. A slow backfill is a cost; a throttled production refresh is an outage.

**Do not splice a different provider into this table.** A `XNAS.ITCH` daily export covering 2018-2026 exists locally and is four times longer, but it is single-venue: a NYSE-listed issue's advance/decline there is decided by its Nasdaq close rather than the consolidated close, and its volume is one venue's slice against this table's whole-market figure. Writing it into a table still being appended daily by the Polygon path would make the series incomparable with itself across the join. If more history is genuinely needed, add a separate table with its own stated provenance and report the two alongside.
...
### 49.3 备份保留的两条不变量（2026-08-09）

`backup_facts.prune_old_runs` 要同时满足两件事，缺一都会静默丢备份：

1. **空壳不占保留名额。** prune 曾只数目录不看内容，于是一次在 `psycopg2.connect()`
   处被中断、只留下目录的运行占掉了 `KEEP_RUNS` 的一个位置——`KEEP=14` 实际只装了 13 份真数据。
   现在先无视年龄清除空壳，再按 keep 淘汰真备份。判断"有内容"必须显式排除 exFAT 的 `._` 影子文件：
   真文件被删、影子留下的目录，用 `ls` 或计数看都像有内容。
2. **运行目录建在数据库连接成功之后。** 此前 mkdir 在前，连库在后，两者之间任何失败都留下空壳。

删除整个运行目录用 `shutil.rmtree`，不要手写 `iterdir()` 循环：`iterdir()` 是对活动目录的惰性生成器，
删掉真文件会连带移除 `._` 影子，生成器推进到已消失的条目时在**迭代处**抛错——
`unlink(missing_ok=True)` 管不到那里。该现象只在 exFAT 出现，APFS 复现不了。
