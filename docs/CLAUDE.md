...
- The refresh worker currently remains single-process. `collector/ecosystem.config.cjs` sets `REFRESH_WORKER_BATCH_SIZE=10` (raised from 2 on 2026-07-24 after a measured ~2.83s/symbol cost), reducing an estimated 81-symbol cold fill from ~41 minutes to ~9 minutes. This is a throughput tuning change, not bounded multi-worker concurrency: do not start a second worker until global derivations and stale-job recovery/deduplication are made singleton-safe. Production requires a collector reload after changing the PM2 environment.
...
- The candidate result ledger (R2.1, `candidate_ledger` + `/api/scanner/ledger` + `/ledger`) is DURABLE and separate from the pruned `scanner_candidate_snapshots` — capture entries there, never try to read outcomes off the pruned snapshots. It scores past candidates at expiry (`evaluateOutcome`): multi-expiry calendars are `not_evaluable` (never guess a far-leg reprice), missing closes are `no_price`, and these are disclosed, not hidden. It is model validation / scoring-weight training data, never a trade signal, and starts empty — fills as candidates expire.
