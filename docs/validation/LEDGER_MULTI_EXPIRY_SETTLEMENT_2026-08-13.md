...
PM2 `quantrift-ledger-far-leg-marks`，首版排在工作日 13:15 PT（= 16:15 ET，最后一轮报价之后）。
**接入第二趟 IB 定向报价后改为 12:45 PT，理由见 §6.5——16:15 ET 已收盘，第二趟永远不会执行。**
...
补跑进行中的中间态（收盘价覆盖 57/312）：

```
evaluateLedger -> { resolved: 0, deferred: 75 }     ← 收盘价未齐的正确推迟，不再写死
```

**这一行就是修复本身的证明**：同样的输入，修复前会写 75 行 `no_price` 并永久定格，
修复后一行不写、留着等数据。

补跑完成后的终态（收盘价覆盖 **312/312**，与平日持平；末尾 6 个失败标的
ACAC/FX/OS/RE/SMS/TTM 与上一轮完全相同，都是失效代码，非新问题）：

```
台账已评分行   15 → 97      （39 win / 58 loss）
no_price       90 → 8       （全部 far_leg_mark_missing）
已过期未结算   0            ← 积压清空
```
...
**56 行多到期日结构真的结算出来了**，而它们在前一天还属于「结构上永远不可评」。
剩下 8 行的 `resolution_reason` 明确是 `far_leg_mark_missing`——就是那 7 个落在
±5% 窗口外的深度实值远腿，即阶段二接手的部分。诊断不再需要反推。

**按策略族的构成整个反转了**：

| strategy_family | 已评分 | 胜率 |
| --- | --- | --- |
| time_spread | **56** | 44.6% |
| single_leg | 25 | 24.0% |
| iron | 9 | 33.3% |
| credit_vertical | 5 | 100% |
| straddle_strangle | 2 | 0% |

原本 0% 可测的族现在是最大的已评分族。
**台账第一次真的在测量引擎推荐最多的那个东西。**

**但这些胜率一个都不能当结论**（与 2026-08-09 记录的同一条纪律）：97 笔里 56 笔来自
08-14 同一个到期日、25 笔来自 07-29 同一天建仓，按独立观测算远不到 97；
`credit_vertical` 的 100% 是 5 笔、`straddle_strangle` 的 0% 是 2 笔。
能做样本外切分的批次在 09-18（123 笔）与 10-16（129 笔）。**在那之前不得改动打分权重。**
...
以下三项在 2026-08-15 已完成，详见对应章节，此处仅留索引：
~~部署~~（§6.2.1）、~~阶段二 IB 定向报价~~（§6.5）、~~交错被抹掉~~（§6.4）。

仍然成立的：

- **历史行不回填**。现存 36 行 `not_evaluable` 的结算日已过、远腿当日报价从未被采集，
  **不得用今天的链倒推**——那正是台账要防的 look-ahead。
  同理 08-14 那 8 行 `far_leg_mark_missing` 也不补：那 7 个合约的 08-14 报价已不存在于任何地方。
- **打分权重不得改动。** 已评分 97 笔，但其中 56 笔共享 08-14 一个到期日、25 笔共享 07-29 一个建仓日，
  独立观测数远低于 97。首批支持样本外切分的是 09-18（123 笔）与 10-16（129 笔）。
  在那之前任何权重改动都无法判断改好还是改坏。
- **下一个检查点 2026-08-28**（147 个合约 / 39 个标的）：两趟采集机制的首次完整运行。
  08-14 那次第二趟尚未写好，41/48 全部来自快照；届时要看两趟合计能否把 85% 推向接近 100%。
- **`quote_watchlist` 与结算标的的重叠仍会漂移**。调度器已能自行发现结算日标的
  （§6.2.3），但每周清单重选仍可能让常规报价覆盖与台账需求错位；
  暂不处理，观察 08-28 那次 `settlement_enqueued` 的实际数量再定。

## 八、方法论小结

三个错误，同一个形状：**把可恢复的缺口登记成不可变的属性**。

| 缺口 | 被登记成 | 实际是 |
| --- | --- | --- |
| 没人去取远腿的价 | `not_evaluable`（结构上不可评） | 没采集，采了就能算 |
| 日线还没发布 | `no_price`（终态，永不重看） | 迟到的数据，等就有 |
| 前 20 名清一色 Diagonal | 看起来是"该策略更好" | 枚举组合数多 16 倍的抽样假象 |

第二个是在修第一个的当天引入放大效应、第二天才发现的。
**修完一个形状，应当立刻沿同一条链路问它有没有第二处**——当时没问。

第三个还多一层：修复代码和它的注释、单测**早就写好并且是对的**，
只是被调用方上一层的一次重排整个抹掉。
**纯函数上的单测无法证明特性在生产路径里生效**，而这是仓库第二次踩它。
