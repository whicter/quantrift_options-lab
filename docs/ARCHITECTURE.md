...
The first manual cloud execution reached PostgreSQL and loaded the 67-symbol watchlist. Investigation found a state-handling defect: a successful TT session exchange can include a successor `remember-token`, but the prior collector retained only the session token. Its next one-shot invocation then used an obsolete credential and failed. The additive migration was applied on 2026-07-16. `provider_auth_state` is shared only by collectors using the same `DATABASE_URL`: a local `auth.py --login` does not seed Railway if its database binding differs. Railway instead bootstraps an empty Railway row from `TT_REMEMBER_TOKEN`; every collector then locks its provider row for the exchange and commits the successor returned by TT before accepting the session. It never creates a token, retries a failed login, or overwrites the row on 401/403. This avoids a Railway Volume entirely. The current token was written to Railway and a post-deployment run was triggered; cloud collection remains incomplete until that run's log and resulting `iv_history`/`provider_auth_state.updated_at` are verified.
