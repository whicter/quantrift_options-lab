...
The first manual cloud execution reached PostgreSQL and loaded the 67-symbol watchlist. Investigation found a state-handling defect: a successful TT session exchange can include a successor `remember-token`, but the prior collector retained only the session token. Its next one-shot invocation then used an obsolete credential and failed. The additive migration was applied on 2026-07-16, and `provider_auth_state` is now the shared token state. `auth.py --login` writes a fresh seed there; every local or Railway collector locks that provider row for the exchange, then commits the successor returned by TT before accepting the session. It never creates a token, retries a failed login, or overwrites the row on 401/403. This avoids a Railway Volume entirely. Cloud collection remains incomplete until a new manual `auth.py --login` seed is stored and one manual run updates `iv_history`.
