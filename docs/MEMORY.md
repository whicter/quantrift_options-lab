...
Derived IV Rank cutover is complete in all control planes: ready symbols are filtered before scheduled TT authentication, skipped by queued metrics jobs, and treated as covered by Analyze. Server 40 and collector 81 tests pass. **The 252-observation gate has since been crossed at scale — measured 2026-08-09: 223 of 302 symbols `iv_rank_ready` on that market date, 229 symbols ready at some point, max 297 observations.** The "0/67 ready, still on Tastytrade cold-start rank" state recorded when this shipped is historical; derived IV Rank is now self-sufficient for the large majority of the universe.
...
Derived volatility 已完成：`volatility_history` 隔离 Polygon HV/ATM IV；API/scanner 输出字段级 provenance。Railway runtime 为 HV 67/67、ATM IV 67/67、ATM DTE 30–43；IV Rank 需要 252 个独立美东交易日，上线当时为 1–2 observations/symbol、0/67 ready，因此暂用 Tastytrade cold-start rank；
**该状态已过期——2026-08-09 实测 223/302 ready、最大 297 observations，自算 IV Rank 已覆盖绝大多数标的。**一次 UTC 午夜 bug 曾把 30 DTE 算成 29 DTE，现统一使用 `America/New_York` market date 并有回归测试。
