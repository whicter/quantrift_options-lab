...
- 2026-07-16 authentication incident: a successful TT session exchange can return a successor `remember-token`. The previous collector discarded that successor, leaving the next run with an invalid old token. `auth.py` now persists only the successor returned by a successful exchange; it never generates a token, retries credentials, or writes state after a failed exchange. Local runs update `collector/.env`. For Railway, attach a persistent `/data` volume and set `TT_REMEMBER_TOKEN_STATE_PATH=/data/tastytrade-remember-token` plus `TT_REMEMBER_TOKEN_STATE_REQUIRED=true`; without the mounted volume, the collector exits before it consumes the seed token. The current token was consumed by the pre-fix probe; run `python auth.py --login` once more, then run `collect.py` directly without a separate session probe.
