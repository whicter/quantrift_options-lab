...
└── docs/              → 全部项目文档的 canonical source（含 AGENTS / CLAUDE / README / wiki / task / learning）
```
...
日常自动续期（通常自动，但可能触发设备挑战）：
  ...
  → bootstrap token 只在数据库没有 row 时使用；已有 database token 返回 401/403 时不再尝试环境 token
  → 网络错误和密码登录绝不自动重试；失败停止并走设备验证/手动登录流程。日志仅记录 token fingerprint 与 consumer
...
| `gex_history` / `gex_strike_history` | GEX 的**持久**历史（无外键，不随 7 天链清理消失）。前者保留全部盘中快照的标量，后者每 `(symbol, market_date, strike)` 一行 | `market_date`, `model_version`, `spot`, `global_gex`, `gamma_flip`, `gamma_regime`, `call_wall`, `put_wall` |
...
- Confluence Zones：`GET /api/analyze/:symbol/confluence` 在服务端将日线 VP、历史 pivot、ATR/EMA/SMA、可选 GEX 墙与 Gamma Flip、250/90 日 Fib 层位，按 `0.5 × ATR14` 半径聚成价格区间。每个区间返回 0–100 的固定先验强度分、`极强/强/中/弱` 标签与逐模块理由。`40/25/15/10/5/5` 是待 G5 回放验证的冷启动权重，不是拟合概率、交易信号或对做市商实际持仓的断言。
- Confluence G5：2026-07-18 对 72 个可回放标的做 5 日前瞻评分，Gamma 置零，控制组是现有单点 pivot S/R 的 `+/-0.5%` 区间。候选模型的守住率较高，但反转召回较低，综合相对变化为 `-2.07%`，没有达到 `>=15%` 且两项均改善的门槛。因此不在 Analyze UI 展示；详细可复现证据见 `docs/validation/CONFLUENCE_G5_2026-07-18.md`。
...
- **全市场日终 Breadth（R2.3，同一接口的 `broad_market`）**：Polygon Grouped Daily 一次取完整交易日，按当日 `type=CS` reference universe 只保留 Nasdaq/NYSE/NYSE American 普通股，排除 ETF/OTC。独立盘后任务计算上涨/下跌/平盘、净上涨、A/D ratio、上涨成交量占比和分交易所明细，写 `market_breadth_daily`；页面只读数据库。有效样本低于 2,000 或前收盘覆盖低于 90% 时整批拒绝，不拿 scan universe 或不完整响应顶替。2026-07-30 Railway 表已迁移，但 collector 尚未配置 `POLYGON_API_KEY`，因此还没有首个真实快照，接口会诚实返回 `missing`。
...
The Railway service `quantrift-metrics-cron`, defined by `collector/railway.metrics.json`, is now the bounded cloud refresh executor. Every five minutes on weekdays it runs `run_railway_refresh_cycle.py`, which schedules a bounded watchlist batch, consumes `provider_fetch_jobs`, then materializes scanner rows. It shares PostgreSQL but does not run IB, the long-lived Mac daemon, heartbeat, or TT metrics authentication. Its Docker build context is repo root, so Dockerfile references and `COPY` sources must be rooted at `collector/...`.

2026-07-16 runtime incident and correction: the service reached PostgreSQL and loaded 67 symbols, but its stored Railway token returned 401. Railway was initially missing `TT_LOGIN`; this is now an explicit credential-contract field and code fails before an HTTP request when it is absent. A Railway token was also persisted with literal wrapping quotes; removing them did not restore the invalid database state. The collector uses `provider_auth_state` as durable state only across processes sharing the same `DATABASE_URL`; a local manual login does not seed Railway if it points at another database. Railway uses `TT_REMEMBER_TOKEN` only to bootstrap an empty row. An existing database token receives exactly one exchange attempt; on 401/403 it does not use the environment seed as a second request. Logs identify the request with an irreversible fingerprint and `railway-metrics-cron` consumer. Railway needs no Volume and does not receive successor values through environment variables.

After a successful local login wrote a fresh token into that shared row, Railway logged the same fingerprint and TT returned `403 device_challenge_required`. This is a provider device-trust boundary, not a PostgreSQL or token-storage failure. The active unattended path is therefore the authenticated Mac Studio collector writing to Railway PostgreSQL; do not run the Railway TT cron until its device challenge is explicitly completed.
...
策略库的主损益图可直接导出为 PNG。导出的是当前 canvas 的渲染结果，包含当前策略腿、Spot、IV shift、DTE 和合约数量参数，不会重新计算或导出页面中的其他面板。文件名按策略 ID 生成，便于保存不同策略的情景图。

### 多个 DTE 损益快照

除当前情景和到期线外，Payoff 图还显示 75%、50% 和 25% 剩余时间的虚线快照。对不同到期日的策略，每条腿独立递减剩余天数；已经到期的腿只保留内在价值。这样 Calendar 和 Diagonal 等跨期结构不会被错误地画成所有腿同时到期。

### 概率锥图

Payoff 图的蓝色阴影是以策略各腿的数量加权 IV、最长 DTE、利率和股息率计算的一个标准差终值价格区间，约对应 68% 的对数正态价格分布。它用于观察价格落点范围，不表示策略获利概率，也不替代右侧基于盈亏平衡点计算的 POP。
...
每个策略项显示 `IV LOW`、`IV MED` 或 `IV HIGH`。这个标签概括该策略模板的常见适用波动率环境，例如 Long Call 偏低 IV、信用收租策略偏高 IV；它不是用户当前输入标的的实时 IV Rank。实时 IV Rank 与 IV/HV 数据只在 Analyze 和 Scan 中展示。
...
3. 常规交易时段 30m OHLCV 的 Volume Profile POC/HVN。
4. 最近高成交量 Swing High/Low 锚定的 Anchored VWAP。
5. 最新 GEX Wall 与独立聚合的 7–60 DTE 最大 OI Wall。
6. 按现价分 support/resistance，再以 `max(0.5 × ATR14, 0.5% × spot)` 聚类。

每个区域返回 `low/high/center`、`score`、`strength`、`distance_pct` 与 evidence。
