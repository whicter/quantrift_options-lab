...
- [ ] 后续（未做）：① 把 `runMaterialization` 接入 collector 调度（`run_railway_refresh_cycle.py` 或加 `scanner_candidate_materialize` job 消费者），否则批次只在手动运行时刷新、enqueue 的 job 会停留在 `queued`——本次是手动跑的一次性批次；② 前端接入 `/api/v1/scanner/candidates`（与 `/api/scan` 切流一并，属 rollout 后续步骤）。
