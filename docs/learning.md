...
- **算不出的结果要标 not_evaluable,不能臆造(R2.1)**:日历/对角多到期结构在近到期日无法用单一收盘价结算(远腿要重定价)。引擎对这类返回 `not_evaluable`、`no_price` 单列披露、不计入胜率,绝不用近似值凑一个"结果"。台账 4,735 候选里 3,495 是 time_spread,如实排除比假装能算更可信。
  - **⚠️ 2026-08-15 更正(见本文档「一个不肯编造数字的决定,可能把可测的东西也一起排除掉」)**:
    "不得用近似值凑结果"这一半**完全正确、至今有效**;
    但由此得出的"所以这类结构永远不可评"是**错的**——远腿不必用模型重定价,
    它在近腿到期当天有真实的可成交报价,去抓就是了。现已实现(`ledger_far_leg_marks`),
    time_spread 从 0% 可测变成最大的已评分族(56 笔)。
    **"不编造"约束的是数据来源,不是问题本身的可解性。**
...
## 一个不肯编造数字的决定，可能把可测的东西也一起排除掉（2026-08-15）

- 2026-07-24 定下的规矩是对的：多到期日结构不能用单一收盘价结算，远腿要重定价，
  所以标 `not_evaluable`、单列披露、不计入胜率，**绝不用近似值凑一个"结果"**。
  这半句至今有效。
- 错的是由它推出的下一步：**"所以这类结构永远不可评"**。
  这个跳跃里藏着一个没被说出口的前提——"重定价"只能靠模型。
  可是远腿在近腿到期当天**有真实的可成交报价**，去抓就行了。
  抓不到才是 `no_price`，抓不抓得到取决于覆盖率，而覆盖率是可以改善的。
- 代价：这条规矩生效期间，台账 60% 的行（而且恰好是引擎排名最高的 60%）
  被登记为"物理上不可测"，长达数月无人再看一眼。修好后 time_spread 立刻
  从 0% 可测变成**最大的已评分族**（56 笔）。
- **教训**：**"不编造"约束的是数据的来源，不是问题本身的可解性。**
  当一条纪律的结论是"这件事做不到"时，要分清它到底否掉的是
  ①一种取值方式（那就换一种），还是②这个量本身不存在（那才是真的做不到）。
  前者被写成后者，纪律就从防线变成了盲区——而且因为它听起来很严谨，没人会去质疑。
- 识别方法：凡是"因为不能 X，所以 Y 不可能"的记录，都去检查 X 是不是唯一的路。
  这里 X = 用模型给远腿定价，Y = 结算多到期日结构；而 X 从来就不是唯一的路。

## 「每日自动落库」没说清是哪一日（2026-08-15）

- `market_breadth_daily` 一直被描述为"每日自动落库"，但实测两次晚间 cron 都拿不到当日 grouped daily（403），写入的是前一日——**任一交易时段内可读的其实是 D−2**。文档没写错，是**没写完**：一个按 `t−1` 定义的特征，在这张表上根本算不出来。**描述数据管道时，"多久更新一次"和"任一时刻能读到哪一天"是两件事，后者才决定它能不能被某个特征使用。**
- 发布时刻我测不出来（周六，夹在 D 日 22:05 ET 与 D+1 17:10 ET 之间）。与其猜，不如**加一发早晨 cron 让它自己回答**——`expected_market_date` 本就把 settle hour 之前解析为前一日，所以零代码改动；拿到就早落库，拿不到就幂等重写，日志里 `settled on` 带不带 `skipping` 就是答案。**能用一次廉价、可逆的观测替代的推断，不要写进设计。**

## 403 和 429 长得不像，但混在一起会得出错误结论（2026-08-15）

- 探测 grouped daily 的历史下界时，第一轮密集请求返回一串 429，夹着两个 403。若不拉开间隔重测，很容易把限速当成权限边界，得出"历史只有 X 年"的错误结论——或者反过来，把权限边界当成偶发限速而反复重试。**探测边界必须慢到确定不是自己造成的。**
- 结果是滚动两年窗口（2024-08-16 通过、2024-02-16 起 403，而今天是 2026-08-15）。**下界随日历滚动，所以硬编码后就信任是错的**，常量要带测定日期，并留一个环境变量让它可被重新探测。

## 口径相同，才是回填的价值所在（2026-08-15）

- 手上有两份历史广度数据：Polygon 只能回两年，Databento 有八年且已在本地。直觉上选八年那份。但 Databento 是 `XNAS.ITCH` **单一场内**，我们的表是 Polygon **合并整合价**——同一只 NYSE 股票的涨跌在两边由不同的收盘价判定，成交量更是一个全市场、一个只有 Nasdaq 那一份。
- **把它写进一张仍在由 Polygon 口径每日追加的表，会造出一条在拼接处自己和自己不可比的序列。** 回填的全部价值就在于"和向前段口径一致、能拼成连续序列"；口径一换，这个价值归零，只剩下更长的数字。
- 所以选了慢的那条（19.5 小时、两年）。**若长度真的不够，正确形态是另建一张标注出处的表并列报告，不是拼接。**

## 回填脚本的四个默认值，每个都对应一种失败（2026-08-15）

- **可续跑**：19.5 小时的任务必然遇到休眠、断连或限速惩罚。跳过已有日期 + 写入幂等，重跑同一条命令即续上——不需要记录断点。
- **从新到旧**：中断时留下的是与现有数据相连的**连续段**，而不是浮在中间的孤岛。同样是跑一半，可用性差别很大。
- **沿用默认限速**：`breadth` 这个 pacing scope 是我们自己的记账，Polygon 按**账户**计量。回填跑过头会让 option/GEX 通道一起吃 429。**回填慢是成本，生产刷新被限流是事故**，两者不对等。
- **不建假日表**：休市日没有 grouped 响应，采集器的回退循环会如实报告并跳过。另建一份日历，等于给"这天开不开市"造第二个真相源，而且它一定会先过期。

## 「这个 provider 慢」之前，先问「我们用满它了吗」（2026-08-15）

- 采集一个标的的期权报价要 165 秒，而同一个账户在 TWS 里手动看整条链是瞬时的。差别不在 IB，在于**我们的适配器一次只订阅一个合约**：`reqMktData` → 最多等 4 秒 → `cancelMktData` → 下一个，180 个合约就是 180 轮串行往返。**IB 每账户约 100 条行情线路，我们同一时刻只用 1 条。** 改成批量后同条件 A/B：249.5s → 14.4s，**17 倍**。
- 最刺眼的是**同一个仓库里已经有正确写法**：`ib_news_provider` 的注释明写「受行情线路上限约束，所以分批订阅，批内并行而不是每标的付一次往返」。期权链这条路径只是从来没按这个模式写过。**引入第二个用同一协议的采集器时，应该先看第一个是怎么做的。**
- 用户的一句「我自己交易时明明很快」比任何 profiling 都直接。**当外部工具在同样的接口上明显更快，那不是它有秘密通道，是我们用错了接口。**

## 基于错误前提的容量结论会长期误导（2026-08-15）

- 配置注释里写着「再往上需要把 quote lane 按 client id 并行，不是把这个数字调大」——**结论是错的，因为前提是串行**。真正的瓶颈不是 client id 数量，是每个 client id 只用了 100 条线路中的 1 条；按 client id 并行只是把一个低效循环复制几份。批量之后，一个 client id 单轮就能覆盖 361 个标的，超过全部 327 个 scan 标的。
- 教训不是"注释写错了"，是**容量结论必须连同它的前提一起记录**。那条注释如实记了实测数字（164.6s/标的），但没记"这是串行实现下的数字"，于是数字本身变成了一条看似客观的天花板。
- 同一段配置里另一条注释也随之作废（"IB 串行，耗时随合约数线性增长，所以扩 DTE 会拖慢报价"）。**改了实现就要回头清理由旧实现推出的所有结论**，否则它们会以"有实测支撑"的姿态继续误导。

## 批量化不能丢掉串行版本辛苦挣来的保证（2026-08-15）

- 原串行代码里有三条注释明确记录着事故换来的行为：**订阅必须被取消**（泄漏一条线路就少一条，且不报错）、**socket 断开要秒级失败**（ibapi 断连不抛异常，只能靠循环自己发现，否则剩余合约每个耗满超时）、**逐到期日失败隔离**（一个到期日超时不能毁掉已采集的其它到期日）。重构最大的风险不是写错新逻辑，是**把这些悄悄丢掉**——它们看起来像是可以简化的防御性代码。
- 取消必须在 `finally` 里覆盖异常路径。批量下这一点比串行更重要：串行泄漏一条线路，批量泄漏一整批。
- 超时语义也变了：串行是"每个合约等 N 秒"，批量是"整批等 N 秒"。**必须显式重新定义什么算完整**——我保留了"未完成的合约仍产出快照"，因为 illiquid 行权价拿不全是常态，丢弃它们会静默收窄链，而调用方的完整性统计本来就在处理这种缺口。

## 一轮扫描如果跑得够久，时间边界会把 universe 切成两半（2026-09-17）

- `collect_prices` 按字母序跑 2h55m，中途横跨 provider 的 end-of-day gate。结果不是"慢"，是**同一轮里 A–T 和 U–Z 拿到了不同答案**：319 个标的只有 52 个当日有 bar，而那 52 个的首字母全在 U–Z。字母表看起来毫无道理地成了一个数据质量维度。
- 这类 bug 的识别特征是**成因与症状之间没有语义关联**。发现时不要先找"这些标的有什么特殊"——先按处理顺序和处理时刻排一遍。写入时间戳给出的切点精确到秒（`00:00:21 ET`），比任何关于标的属性的假设都快。
- 根治不是把那一轮挪到 gate 之后（那只是让切点落在轮次之外，轮次一变长又会重新跨回去），而是**换成一次调用覆盖全体**的接口。grouped daily 的价值不在于快 300 倍，在于它**不可切分**：一个 session 要么对所有标的落地、要么一个都没有。
- 一般化：**任何"逐个处理 N 个对象"的批作业，只要运行时长可能跨越一个外部状态变更点，就会把对象集合按处理顺序切成语义不同的两段。** 要么让作业短到不可能跨越，要么让它对那个状态变更免疫。

## 断言 provider 行为之前，先让 provider 自己说（2026-09-17）

- `ecosystem.config.cjs` 的注释断言 "18:35 PT (= 21:35 ET, past the EOD settle)"，`PRICE_EOD_SETTLE_HOUR_ET=20` 把同一个断言编码成常量，还有一条单测把它固化成"Thursday 21:35 ET → 当天的 bar 应该已 finalize"。**三处互相印证，没有一处来自 provider。**
- 一次请求就推翻了全部三处：`403 NOT_AUTHORIZED — "Attempted to request today's data before end of day."` 这不是"发布慢"，是套餐条款。我先前把它推断成"Polygon 午夜发布"，方向对但性质错——**性质错会导致改法错**：如果只是发布慢，等一等就行；是条款，就得知道条款的边界在哪，以及它永远不会因为等待而改变。
- 单测把错误假设固化下来，反而让它更难被质疑——三处一致会读作"已验证"。**测试只能证明代码符合我们的假设，不能证明假设符合外部世界。** 涉及外部系统行为的常量，注释里要写清它是**测出来的**还是**推出来的**，以及测的那一次是什么时候。

## 一个月前就做过的实验，结果躺在日志里没人读（2026-09-17）

- 查日线为什么落后时，`ecosystem.config.cjs` 里 breadth 那条线的注释已经写着完全相同的发现（"both the 20:05 and 22:05 runs took a 403 on that session"），还专门加了一班 06:05 ET **就是为了测"Polygon 是否隔夜发布"**，并在注释里写明了判读方法："a morning line with no `skipping` means D-1 was available"。
- 我第一次去读，grep 的是 `settled on`，结果得到"09-09 之后凌晨那班就不跑了"的错误印象——因为那行**只在需要回退时才打**。按注释说的去读（看有没有 `skipping`），答案是它**连续两周每天都拿到 D-1**。**注释告诉了我怎么读，我却按自己的猜测去 grep。**
- 两个教训。一，**相邻通道踩过同一个坑时，结论往往不会自己传播过来**：breadth 记下了 403，`collect_prices` 的注释仍然断言 21:35 ET 已过 settle，两者隔了一个月、在同一个文件里。二，**日志里"没有某类行"不等于"没发生"**——只有先弄清成功路径打什么、失败路径打什么，缺失才有意义。
- 实践上：动一个 provider 相关的时间参数之前，先在仓库里搜同一个 provider 的其它调用点。这次那里正好有一个已经跑了两周的现成实验。

## 固定成本的摊销比例，才是批大小的真正含义（2026-09-17）

- 期权链刷新只有 69 job/小时，第一反应是并发不够或 provider 限速。实测拆开：10 个 job 花 112s，之后那次全 universe 派生花 **368s**，占周期 76%，**且与 batch 里有多少 job 完全无关**。通道 75% 的时间空转在等一个空队列。
- 所以 `REFRESH_WORKER_BATCH_SIZE` 不是"一次抓多少"的调速旋钮，它是**固定成本的分母**。10 → 30 不增加任何 provider 压力、不改执行并发，却把吞吐翻倍，顺带把派生频次降下来。
- 诊断顺序的教训：**先量"忙 vs 空"，再量"忙的时候在忙什么"**。直接去量并发或 job 时长，会得到一堆正确但无关的数字——那 112s 无论怎么优化，都动不了占 76% 的那一块。
- 中途踩了一个测量陷阱：按 `started_at` 重叠算并发得到 max=10，与配置的 3 矛盾。原因是 `fetch_jobs` 在 **claim 时**给整批统一打 `started_at`。**用来划分阶段的时间戳，未必是该阶段真正开始的时刻**——用它之前先看写它的那行代码。

## 「只增不减」的同步脚本会撤销每一次人工决定（2026-09-17）

- `sync_universe.py` 的 upsert 在 `ON CONFLICT` 里无条件 `active = TRUE`。代码库里**没有任何地方自动把 `active` 置 FALSE**——它只会被人为退役置 FALSE——而种子集合取自 `iv_history ∪ price_history ∪ option_chain_snapshots`，恰恰保留着这些"因为产不出东西而被退役"的 ticker 的历史行。于是每跑一次 sync，就把每一次退役撤销一次。
- 危险在于**它不报错、也不留痕**：标的悄悄回到 universe，没有任何记录说明它为什么回来，下一个人还得重新验一遍它是不是死票。2026-07-30 那次退役已经在提交信息里记下了这个坑，但当时是按单个标的绕过，根没有修。**绕过一个已知陷阱而不修它，等于把它留给下一次。**
- 一般化：**幂等的同步脚本和人工的状态决定放在同一个字段上，必然冲突。** 判断标准很简单——问"这个字段有没有自动置反的路径"。没有，就说明它承载的是人工意图，同步脚本就不该写它。

## `pm2 save` 过了，cron 注册仍可能在守护进程重启时丢掉（2026-09-23）

- 现象：PM2 守护进程 07:41 重启后，14 个 cron app 里只有 `news`（`*/5`）还在触发，其余 13 个——包括每小时跑的 `log-rotate`——全部停在 07:20 之前。**`pm2 list` 里它们状态正常、`cron_restart` 字段也在**，只是永远不会再被触发。直接后果是当天的价格采集根本没启动，一整天的日线缺失。
- 所以 **"进程在列表里" 不等于 "它还会被触发"**。判定 cron 型 app 是否健康，唯一可靠的信号是**最后一次实际日志写入时刻对不对得上它的 cron 表达式**——把所有 cron app 的 `cron_restart` 和日志 mtime 并排列出来，异常会立刻显形（那次是切点整齐地落在 07:00–07:20，紧邻守护进程重启）。
- 这条和仓库里已有的那条（`pm2 restart` 不读 `ecosystem.config.cjs`）是同一类问题的两面：**PM2 的运行时状态与配置文件之间没有任何自动一致性保证**，两边都可能单方面漂移。`check_pm2_env_drift.cjs` 查的是 env 漂移，**触发漂移没人查**。
- 补救动作是同一套：`pm2 delete` + `pm2 start ecosystem.config.cjs --only <name>` + `pm2 save`。分组做，并且先列一遍同机上**不属于本仓库**的常驻进程，确认删除范围不会波及它们——这台机器上 49 个 app 里有 12 个是别的仓库的。

## 修复"已生效"的证据，可能来自另一个还在跑旧代码的进程（2026-09-24）

- dedupe 修复上线后告警还在响。表面看像修复没用；实际上**新代码完全正确**——它对单 issue 那组算出的 `4a993106` 与库里那行逐位吻合，那行活了 5 小时只推了一次。同时库里另一组 issue 每 5 分钟冒一个新指纹，而**新代码对那组算出的值永远是 `07d0eea6`**，换标的换数值都不变。库里那些指纹新代码根本产生不出来。
- 所以判据是：**用当前代码对观测到的输入重算一遍输出，对不上就说明观测不是当前代码产生的。** 这比看配置、看 `pm2 list` 都直接——那两处都显示"一切正常"。
- 根因是 PM2 守护进程重启时脱管的孤儿（PPID=1）。它们不在 PM2 的进程表里，所以 `pm2 delete` + `pm2 start` 不但没替换掉它们，反而**在旁边又起了一份**。重新注册之后必须用 `ps` 按脚本路径核对实例数，而不是只看 PM2 说了什么。
- 同一次重启丢了两样东西（cron 注册 + 进程句柄），我第一次只发现了一样。**一个根因造成的多个症状，修完一个之后要回头问"它还可能弄坏什么"。**
